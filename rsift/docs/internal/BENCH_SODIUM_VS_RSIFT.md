...
(RD24 では 7,980 編集で差分 5.717 s/114,278,592 B = 全量 C (36.013 s/1,522,835,136 B) 比時間 6.3x・bytes は全量の 7.5%、B (16.812 s/6,360,512,960 B) 比時間 2.9x・1.8% — §9 表)。
...
| 頂点キャッシュ品質 (ACMR) | **C Rsift** | 36c: 1.669→0.929 / RD24: 1.672→1.030 (A/B はエンコードのみで最適化なし。差分維持パスは dead-tri 償却の帰結で 36c 1.049 (全量 0.858) / RD24 1.045 (全量 0.889) = 全量再最適化より悪 → wave 215/216/217 誠実記録・§8 backlog) |
| リージョン I/O 時間 | **C Rsift** | 7.6 ms vs 46.8 ms (6.1x) / RD24: 586.7 ms vs 3,281 ms (5.6x) |
| 実体の真の遮蔽カリング | **C Rsift** | 600→125 (B は FOV までの 422, A は 600。DDA 遮蔽純粋比較の legacy 基準 175 から wave 214 V2 は時間 4.9x で実質同等深カリング。RD24 正規値 (40,016 体, wave 217 HN): B 601 @173.4 µs vs C 167 @1,029.8 µs = 時間 B 勝/深度 C 勝 — §9.3) |
...
| 編集ワークロード (時間/再生成バイト) | **C Rsift 差分** (wave 215) | 86.7 ms (C 全量比 7.7x・A 同等 88.6 ms) / 1.56 MB (A の 0.9%・C 全量の 6.4%)。RD24 (7,980 編集, wave 217 HN): 差分 5.717 s/114,278,592 B = B 16.812 s 比時間 2.9x・bytes B の 1.8% (A 4.644 s が最速 = 高密度帯 reopt 2,941 回、誠実併記)。面集合パリティ touched 9,425/9,425 不一致 0 = 出力 bit 同一。差分 ACMR 悪 (36c +0.191 / RD24 +0.156) は誠実併記 (§5.7/§9.3) |
...
- 差分メッシュ (wave 215) の ACMR 改善 (36c 1.049→全量級 0.858 / RD24 1.045→0.889
  へ): dead-tri 蓄積期の頂点局所性劣化に対し、reopt 閾値の
...
trade-off) と #7 (実体時間 = B 錐台+距離のみ vs C 3,000 レイ真遮蔽の設計差、wave 217 HN で 40,016 体へ正規化済・µs 級の絶対差) に
