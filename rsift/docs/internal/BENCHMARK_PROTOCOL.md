...
- **シム錄画 (3者共通・サーバ側コスト)**: 240 tick。モブは徘徊AI・重力・AABB衝突・水浮力を実計算。距離 96 超でデスポーン、カメラ周辺に補充スポーン (本家モブキャップ式)。スポーンの **40% は地下洞窟** (遮蔽が意味を持つ環境)。モブの草踏み荒らしで毎tick最大3ブロックを改変→セクション汚染、60 tick 毎にオートセーブ
