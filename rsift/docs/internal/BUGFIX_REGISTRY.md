...
| GM-1 | wave 193 | rsift-opt-gfx/apple_backend.rs + gpu_runtime.rs (request_adapter site) | **[機能]** MacBook dual-GPU (Intel 世代) で adapter 選択が `RequestAdapterOptions::default()` 丸投げ (PowerPreference::None=列挙最小 id) のまま policy 経路なし — 「軽い」iGPU 経路を開く装置が不在 (ユーザー方針: 本当に軽くなるデバイス依存は保持、Metal4/GL 削除は誤解で apple_backend 健在を機械反証済) | **採番なし (機能開発):** GpuPowerPolicy {HighPerformance/LowPowerPreferred/WgpuDefault} + adapter_power_policy (RSIFT_GFX_POWER 正規化 trim/lowercase、low→iGPU 側・high・default・未知値既定再帰・None→HighPerformance 明示化) + gpu_runtime に map_power_policy (3 腕)・adapter_options_with_power_policy (env 1 回読み+info! 可観測化) を新設し request_adapter site 置換 — vendor 一次情報: wgpu-types:141-149 PowerPreference・wgpu:1386 alias 2 lifetime・wgpu-core instance.rs:923-933 LowPower→integrated 優先規則・wgpu-hal metal adapter.rs:610 low_power。単一 GPU (Apple Silicon 全機) では or-連鎖が唯一候補収束で no-op 非破壊。TDD: compile RED (E0432/E0425/E0433)→GREEN 3/3。strict +3 = 1431 (gm_adapter_power_policy_golden 行列+junk 再帰・gm_wgpu_power_mapping_golden 3 腕・gm_adapter_site_power_policy_lexeme split concat! 自己言及対策)。adversarial 4 系統全 RED (A "low"腕 swap・B 未知値再帰削除・C map 腕改竄・D site default() revert)、復元 MD5-VERIFIED×4。自己照査: 構造破壊 (幻影 validate ループ憶測混入) を bak 一次参照で忠実修復・E0107 (2 lifetime)・E0425×3 (super:: 修飾)・fmt 1 行逸脱外科修正、全誠実記録。実効 (iGPU 発熱/バッテリー) は sandbox/CI 非計測の誠実境界明記。api 49・replay 16・警告 0・fmt 自己起因 0 |
| GN-1 | wave 194 | rsift-opt-gfx/objc_rt.rs + metal_direct.rs + apple_canon.rs + apple_ffi_audit.rs (全新設) + apple_backend.rs + lib.rs + tools/apple_canon_gen.py (新設) | **[機能]** ユーザー要求「METAL、GL 完全実装・直 binding・Mac 実機なしで METAL API ドキュメント正典の完璧な独自静的解析マシンを Rust で作り実際に動作するかある程度検証して動くことは保証して」| **採番なし (機能開発 wave):** 3 層保証機械の実装 — (1) canon 正典表 (apple_canon.rs 機械生成、vendor objc2-metal/foundation/quartz-core の Apple SDK ヘッダ header-translator 転記から 7840 selectors・92 enums・7 extern C fns・2614 parents、重複 0) + generator 4 修正 (impl-for/bare-impl 帰属で Foundation 帰属欠損根治・Apple 命名 family 規則 retain 正規化・`#[unsafe(super(X))]` super 辺 +148・NSOBJCORE init) + 検証 60/60 MISSING 0・retain 7/7。(2) 独自静的解析機 apple_ffi_audit (R1 SEL canon 継承解決存在・R2 enum 値厳密・R3 コロン数=trait 引数=呼出引数 三者一致 80 呼出・R4 extern link_name 正典・R5 fixture 所有規則 canon 整合の mock 自己照査・R6 消費者ゼロ宣言禁止機械化・R7 CLASS canon 解決、論理フラグメント+cursor 構文駆動で rustfmt 非依存、run_full_audit 0 件 pin + 変異 11 本で各規則 RED 証明)。(3) metal_direct classic Metal 完全実装 (objc_msgSend typed transmute 直 binding、高級ラッパ不使用) — create/create_with_format(RGBA8 検証)・update_vertices (newBufferWithLength 容量固定+contents 直書き Apple 動的データ定石、mock arena 64KiB で物理 memcpy 検証)・probe_caps・render_frame/_region (scissor 部分描画)・render_textured_quad (lazy 第 2 pipeline)・render_frame_indexed (UInt16)・create_index_buffer_u16・upload_pixels・readback (device 真値 guard+clamp)・attach_layer・render_to_layer・shutdown、全経路 FrameSpec/encode_frame 単一核収斂 + MockObjcRt (init absorb・live 追跡・u64/ptr override) による本番全行 Linux 動的検証 (呼出列・リーク 0)。apple_backend に NativeDirectPlan (RSIFT_GFX_APPLE_NATIVE+ffi_audit_clean ゲート) / ffi_audit_report / #[cfg(macos)] native_direct_session 実配線。strict +32 = 1463 全緑 (gn_ prefix 32 本個別フィルタ確認: objc_rt 3・metal_direct 16・audit 11・apple_backend 2)・警告 0・fmt 6 ファイル 0 逸脱 (rustfmt::skip を生成器組込み)・簡体字 0。adversarial 4 系統 5 RED (A canon 1 行削除 R1・B SEL typo R1・C dispatcher 差替はコンパイラ E0061 捕捉を誠実記録・C' コンパイル通過 SEL 参照差替 R3・D shutdown release 1 行削除で mock live [12] 検出)、復元 MD5-VERIFIED×4 系統。自己照査 10 件誠実記録 (mock u64 status override 追加・fixture accessory getter の Owned 誤登録 を リーク検出捕捉→canon Borrowed 修正 (本番正しく mock 違反症候)・13 定数消費者ゼロ → 全真 wiring・calls.clear() 漏れ・describe 誤変更復元・audit 初版オフセット/R6 substring 偽陰性/link_name base/resolve leak 修正)。実機 GPU 応答・macOS リンクは sandbox 非計測の誠実境界明記 (apple rust-std 取得 curl: (35) 記録)、GO(195)=MTL4・GP(196)=CGL/GL 計画行。api 49・replay 16 |
| GQ-1 | wave 195 | rsift-api/mod_security.rs (新設) + native_loader.rs + mod_dispatch.rs + mod_api.rs + mod_menu.rs (テスト構築点) + lib.rs | **[機能]** ユーザー要求「Mod がユーザーの PC を乗っ取れないようにセーフティ (ハッキング防止)、ただし作れる Mod の幅は狭めるな」— プロセス内ネイティブ Mod のサイレント乗っ取り経路 (外部プロセス起動・機密ファイル窃取・外部通信・インジェクション・永続化・キーログ) がロード前検査不在で無防備 | **採番なし (機能開発 wave):** 多層防御の実装 — (1) HostCapability 8 種の能力モデル + 2 層静的検査 (PE import 構造パース手書き全境界検査 + PE/ELF/Mach-O 横断文字列シグネチャ) `vet_bytes` 純粋関数 (2) 窃取ツール語彙 (mimikatz/sekurlsa/lsass 等) + インジェクション連鎖 7 語彙 2 種共起の即時 Deny (3) SHA-256 自前実装 (NIST 4 ベクタ) ハッシュ固定の同意ストア `.rsift_mod_perms.json` (Mod 差替え=同意自動失効・least-privilege 付与・未承認時 scaffold 自動記帳) (4) SecurityGate ランタイム能力検査 + 監査 ring (容量 64) + snapshot/report 観測 (5) JNI ブリッジ帰属不能を踏まえた `host.` 接頭辞 op 構造拒否 (拒否計数つき)。配線: load_single_mod で動的リンク以前に vet (Deny/RequireConsent=中止・他 Mod 継続)、ModManifest.capabilities 記録、dispatch_op 最前段ガード。**幅不変の機械 pin**: 検出 0 Mod=即 Allow 摩擦ゼロ・SuiteModule 42/op 4 無制限維持 lock。strict rsift-api 66 全緑 (gq_ 17 本個別確認: sha256 NIST 4・PE parse/classify・benign clean・malformed 全截断 no-panic・injection chain・盗取語彙・string scan・manifest・verdict lattice 5 腕・gate audit/ring・consent tamper・jni prefix・breadth lock・dispatch 計数・sidecar scaffold・format・loader 統合)・opt-gfx 1463 不変・警告 0・fmt 新規 0/既存自己起因 0 (native_loader CRLF 原生 34=34 LF 正規化同一条件)・簡体字 0。adversarial 4 系統 6 RED (A needle 除去 1・B chain 閾値 1・C hash 照合無効 2・D op ガード常許可 2) 復元 MD5-VERIFIED×4 (新規未追跡ファイルの git restore 不能を needle 再挿入+md5 で正確復元の自己照査 1 件)。機械値自己照査: ALL 44 暗算誤記を pin RED 捕捉→実値 42。誠実境界: シグネチャ未収載ロジック/承認済信頼 Mod の行動/難読化ペイロードは保証外 (WASM ティア併用推奨)。GO/GP は 196/197 へ繰延明記 |
| GO-1 | wave 196 | rsift-opt-gfx/metal4_direct.rs (新設) + objc_rt.rs + metal_direct.rs + apple_ffi_audit.rs + apple_backend.rs + apple_canon.rs + lib.rs + tools/apple_canon_gen.py | **[機能]** ユーザー要求「METAL、GL 完全実装 (最新 MacBook は Metal 4 直 binding)」の MTL4 部 — wave 194 計画行 GO の実行 | **採番なし (機能開発 wave):** (1) Apple SDK 26.5 一次情報 canon 化 — Metal.framework 全 98 ヘッダを gh api raw 取得 (rsift-scratch/sdk26 保存)・apple_canon_gen.py SDK26 parser (状態機コメント除去・availability マクロ 599 個 paren 平衡除去・enum 定数式 fixpoint eval・i64 two's complement wrap・LCP variant 命名) で CANON_SDK26_SELS **2476** / ENUMS **902** / CLASSES **231** / PARENTS **316** 生成、idempotent md5=668e91f1(2 連続一致)、既存 4 区画無改変。一次情報確定事実: maxBufferBindCount 上限 31 (記憶値 128 の誤り捕捉)・Metal4=5002・MTLStages ビット位置・gpuResourceID は MTLTexture 帰属。(2) DirectMetal4 完全実装 (1301 行) — SEL_MTL4_* 42・MTLV_* 4・CLASS 7・frame 正規順序 (wait→reset→begin→pass 都度生成→encoder→setAddress/atIndex→setArgumentTable/atStages→draw→end→commit:count:1→signalEvent)・residency/present 経路・所有規則 canon 厳密 (pass descriptor frame 毎 alloc→release)。objc_rt に void_2uu/bool_2uu/u64_value_log/on_u64 後勝ち根治/on_ptr_off・bool_1u override 駆動 (全部 Native transmute+Mock 双実装)。(3) 監査機 union 拡張 — R1/R2/R7 SDK26 chain・audit_dispatch_shapes_with_decls (層連結 decl 解決で跨ファイル SEL 参照を形式化)・logical_statements char リテラル判別根治 (lifetime 'static の ' で in_char 誤入 → SEL 42 中 41 抽出の監査陰性化バグを捕捉根治)。(4) apple_backend native_direct4_route (4 条件 AND policy、Linux pin 可能) + #[cfg(macos)] native_direct4_session (§7 真消費者)。strict 1463 (stash 機械確定) → **1480 全緑** (go_ 17 本新設=metal4 9/audit 7/backend 1; go_ フィルタ 21 表示は pgo_bolt 4 部分一致併計で初版誤計上を #[test] 件数交差検証で根治・誠実記録)。R3 修復履歴 (wave 内完結⑧): per-file 走査化で顕在化した 3 失敗 (=gn_audit_full_clean/report/plan_matrix) 根治 (decl_merged・SEL_CONTENTS use 統一・ptr_off→on_ptr_off 命名衝突根治)。adversarial 4 系統全 RED→復元 MD5-VERIFIED×4 (A canon 値改竄→go_canon RED・B SEL typo→R1 RED・C dispatcher SEL 差替→R3 RED・D frame-sync >=→>→go_frame4 RED; A/B 初回 sed パタン不一致の未適用を自己照査捕捉)。api 66・replay 16・警告 0・fmt 0・簡体字 0・seal 全 PASS (digest 004c1cf5fb17bfe8 rows=357 不変)。実機 GPU 応答非計測の誠実境界明記。GP (197)=CGL/GL 計画行 |
| GS-1 | wave 197 | rsift-setup (新設・std オンリー) | セットアップ bootstrapper (.exe/.app ドロップイン): 同階層 dll/dylib 走査 + SHA-256 検証 (自前実装 NIST pin) + renderer 経路決定 (Metal4/classic/Vulkan/DX12、誤 ready 構造拒否) + rsift_launch.json 生成 + 全工程 self-log (txt/jsonl)。adversarial 系統A (metal4 arch 要件無効化) で既存網不発を捕捉 → 非対応ホスト+metal4 lib 併存網を追加 (20→20 全緑・2 回目で RED 確定)、系統B (ログ schema 改竄) RED。復元 MD5-VERIFIED×2。fmt 0・簡体字 0・opt-gfx 1480/api 66 不変 |
