...
## wave 204 GZ (2026-08-02) — bootstrap jar 未同梱の根治 (実機「Mods ボタン無し」の構造的欠陥)

- 対象: TRIGGER2.md (3 OS zip 構成)、rsift-setup (BOOTSTRAP_JAR/probe_bootstrap_jar/launcher+prism gating/natives)、rsift-jvm/mod_bridge.rs (PacketTap 例外クリア)、docs/SETUP_BOOTSTRAPPER_JA.md
- ユーザー実機報告 (逐語): 「これさ、しっかりMod読み込まれてなくね？最初の画面にModsボタンすらないし。ローダーは認識されてる。」— push 済み配布系統の一次コードを全点検し構造的欠陥 2 件を確定
- 欠陥 (1) [大]: `rsift-bootstrap.jar` (Java ブリッジ実体: RsiftHooks/ScreenInitPatcher/RsiftModBridge/RsiftPlatformBridge 等) が **TRIGGER2 zip 構成に未含有** (7 ファイル定数の範囲外)。setup の scan_libs は dll/dylib/so 拡張子フィルタで jar を拾わず、natives 配置ループも LIB_* 5 名のみ → ユーザ環境のどこにも jar が無い。エージェントの探索 (screen_inject::bootstrap_jar_path = agent dll 同階層 + "rsift-bootstrap.jar") は必ず欠落し、ensure_injector_loaded / ensure_mod_bridge / ensure_platform_bridge が全て "bootstrap jar missing" で静黙失敗 → **Minecraft へのフック注入 0 → タイトル画面に Mods ボタンが現れない・Mod はネイティブロードされても発火点 (nativeOnHook) がゼロ**。ユーザー観測 (ローダー認識=profile/instance 登録成功・ボタン無し) と完全一致
- 欠陥 (2) [中]: mod_bridge の RsiftPacketTap 任意ロード失敗 (同梱 jar にクラス未収録) で pending 例外を残留。添付 boot スレッドに Java フレームは無いため後続 JNI 呼出しが全て暗黙失敗する構造 (ensure_* 完了後の transformer native 登録等)。jar 同梱後も顕在化確定だったため同 wave で根治 (exception_clear + 無効明示ログ)。残 5 箇所の `let _ = env.call*` は Java フック内呼出しで pending 例外はゲームログへ伝播 = 設計上 fail-loud、別クラスとして記録
- 根治 (A): TRIGGER2 run 12、3 OS 全 zip に rsift/bootstrap/prebuilt/rsift-bootstrap.jar (44,301B・git 追跡実物: 20 entries = manifest + 17 クラス + inner 2。RsiftPacketTap.class 未収録は任意機能として継続記録) を同梱 (mac は .app/Contents/MacOS)
- 根治 (B): rsift-setup — BOOTSTRAP_JAR 定数 + probe_bootstrap_jar (Present/Missing/Corrupt 3 値; PK\x03\x04 マジック + 下限 512B、152B manifest-only スタブ型破損を排除; 読取不可は Err で静黙化しない) + setup_launcher/setup_prism 両経路で **jar 必須化** (不在/破損なら登録中止+理由 note。「ゲームは起動するが Mod が全く効かない」壊れた起動構成を量産しない — wave 199 の agent 不在と同じ fail-loud 原則の拡張) + natives へバイト一致配置 + sha256 を notes 記録
- TDD: RED 6 (launcher full jar assert/launcher nojar/launcher badjar/prism full 6 点長+assert/prism jar 配置/prism nojar) → 実装後 GREEN **39** (+5: probe 3 値 machine 検算・launcher missing/corrupt・prism missing・prism jar バイト一致配置)。既存 34 は fixture に jar 追加で意味不変維持
- adversarial 3 系統: (1) probe PK マジック検査除去 → 初回 **非検出** (既存 Corrupt 2 ケースが下限サイズ併発で PK 独立 pin 不在) を捕捉し「サイズ十分・PK 無し」独立 pin をテスト追加 (wave 内自己照査完結) → 再変異 RED 1 検出化 (2) launcher jar ゲート素通し → RED 2 (3) prism jar 配置抹消 → RED 2。復元 MD5-VERIFIED×3。mod_bridge 例外クリアは JNI 環境依存のため unit 変異証明不能 = レビュー confirmed + 実機ログ確認予定の誠実記録
...
## wave 215 HL (2026-08-03) — 編集ワークロード 840 → 84.7 ms (9.9x・A 同等速・面集合 bit 同一) = per-block 差分メッシュ DiffSectionMesh — ユーザー指示「完全再描画じゃなくて差分描画」の正統実装

- 背景: ユーザー指示「遅い場所を大量に修正。完全再描画じゃなくて差分描画でいいかも。スタブ・仮実装・配線なし一切禁止」。bench 編集ワークロード (編集 120 / 汚染セクション延べ 787 = touched 実セクション 136) で C 行が 840 ms、A (85 ms) の 9.9 倍であることを機械確定。temp instrumentation (RSIFT_TIPSIFY_US、後に完全撤収) で犯行現場の内訳を確定: Tipsify CPU 累積 967,363 µs / 787 calls ≈ 1.23 ms/セクション ≈ 全コスト + セクション毎の slot 配列 (17³×27 = 132,651 u32 = 0.53 MB) 全初期化 ≈ 417 MB 書込/全編集。**1 ブロックの編集のたびにセクション 16³ 全面を再走査し Tipsify まで全回しする全量再描画こそが構造的浪費**
- 根治 (DiffSectionMesh 新設、`crates/rsift-opt-gfx/src/diff_mesh.rs`): (a) 編集 1 件あたり編集ブロック+6 近傍の ≤42 面のみ再評価 (実測 4,986 面 vs 全量セクション当たり最大 24,576 面 = 条件評価を 1/130 に) (b) slot 初期化を世代スタンプ比較に置換 (0.53 MB memset を払わない) (c) 除去面は**頂点署名** (面の 4 local id) による値検索で特定し dead-tri (全頂点同一の零面積三角形 = GPU が自動破棄) へ退避 — 「位置台帳」案は Tipsify が index 列を任意に並替えるため再最適化のたび即不整合になることを自検出し設計変更 (設計ノートをコード先頭に固定、kill 走査が found!=2 なら内部不整合 assert で停止) (d) dead 率 12.5% (`REOPT_DEAD_PERMILLE=125`) または 16 連続編集 (`REOPT_EDITS=16`) で一次再最適化 = 頂点 usage 圧縮 (再 encode なしの 12B コピー = 決定的) + Tipsify (実測 reopts=38 / 136 セクション)
- 誠実注記 (悪化部位・隠蔽なし): 差分維持中の ACMR=1.049 は全量+Tipsify の ACMR=0.858 より **+0.191 悪い** — dead-tri 償却期間は頂点キャッシュ局所性が逐次劣化し reopt 時に回復する設計の帰結。改善 (閾値動的化・局所 index 再編・dead 頂点即再利用) は BENCH §8 backlog へ
- 検証 (全機械): TDD RED×6 → GREEN、opt-gfx **1492/1492** 全緑 (+6)。機械保証テスト: 連続 40 編集+reopt 各点で全 key 空間 24,576 を FULL rescan と照合 = bit 一致、add→remove で faces=0 かつ全三角形 dead、dead 台帳≡実 zero-area 数、dedup 共有頂点台帳 (内部点 (4,8,4) = 20 頂点)、セクション外基底の面は絶対生成しない、reopt 圧縮後も面集合保存+slot 新世代整合。adversarial 実効変異 (kill_face の第 2 三角形照合打切) = **RED×5** (内部不整合 assert が設計どおり全件捕捉) + MD5-VERIFIED×1 ファイル。bench 停止ガード: 面集合パリティ touched 136 セクション不一致 0 を `assert_eq` で強制。最終計測 (fmt 後の独立 run で可再現): C 全量 840.0 ms / 24.51 MB → C 差分 **84.7 ms / 1.56 MB** = A 85.5 ms と同速度・bytes は C 全量の 6.4%・A の 0.9% (bytes・パリティ・ACMR は決定的、時間のみ run 間 ±3-5%)
- 誠実境界: 再構築バイト計上は頂点バイト再書込累積のみ (kill 時の index インプレース書換 24 B/面は全量側 index も未計上で両 pipe 同一基準)。差分経路の初回タッチは build_full (≈全量 1 回分と同コスト) で bench は warmup 非計測 = timed replay は編集 2 巡目以降の償却領域 (BENCH §7-8 に明記)。本 wave の消費者は pseudo_mc_bench の `edit_sim_diff` (実配線・parity 検査付き)、本番 full_graph_wiring への転用は別 wave の課題

## wave 216 HM (2026-08-03) — ユーザー主命題「Web 検索で信頼できるソースから Sodium の 2000 項目前後の実測値を取得し、それと全く同じ内容で RsGraphics のベンチを回して Sodium 以上に」への完遂: RD24 (2,401 チャンク = CaffeineMC 公式測定条件) シナリオ新設 + メッシュ逆転 (HM-1) + ソート両スケール勝利 (HM-2) + 可視性 graph 採用 (HM-3)

- 命題の確定 (一次ソース): 「2000 項目前後」= CaffeineMC 公式 Sodium ページ (curseforge.com/minecraft/mc-mods/sodium) 本文脚注 [^1] の測定条件「render distance of 24 chunks」→ (2×24+1)² = **2,401 チャンク**。HW は i7-1165G7 + Xe 96EU (低スペック iGPU 帯域律速 = Rsift の設計前提と一致)。クレームは比率のみ (「up to 23% faster」) で FPS 絶対値は公式非掲載、絶対値は corroboration 源 (note.com/yumu25: d=32 で vanilla 89 → Sodium 433 FPS 平均 等 3 点) に限定。sandbox は GPU 非所持のため絶対 FPS 対決は物理的に不可能 → 「Sodium 以上」は**同一 2,401 チャンク内容ワークロードの計測可能全指標で C ≥ B** と定義 (BENCH §9 に判定表 19 項目)
- ハーネス再構成: 単一ファイルだった `pseudo_mc_bench.rs` (2,400 行) を `examples/shared/pseudo_mc_core.rs` へ分離、**2 wrapper (6x6 / 49x49) が CHUNKS_X/Z だけ差し替えて `include!`** = 測定ロジックの重複 0 で「全く同じ内容」の再現を構造保証。リージョン I/O は Anvil 32x32 境界で複数リージョンファイル分割 (36c は 1 ファイルで従来値不変)。RD24: world gen 1.149 s (784x784x192 = 118,013,952 ブロック実生成、近似タイル無し) / 28,812 セクション / 水クアッド 6,617,899 個
- 回帰ネット: リファクタ基準 run の構造値 (頂点 861,656/bytes/可視数/誤順カウント/rays) と各変更後 run の差分照合を各段で実施。ソートは指紋 (X/Y (Z%) 抽出列) の diff で bit 一致を事前宣言規約として運用
- **HM-1 (メッシュ最適化 = mesh-loop 96.6→42.4 ms (36c 2.3x) / 5.40→2.85 s (RD24))**: 犯行現場は slot テーブル memset (17³×27 = 132,651 u32) と全候補頂点 encode (純粋関数のため省略して意味論不変)。根治: `SlotScratch` 世代スタンプ (wrap 時のみ唯一 fill) + **SLOT_N 132,651→29,478** (軸 6 法線のみ使用 = L2 常駐) + **encode-on-miss**。適用は全 3 メッシュ経路 (remesh_rsift/rsift_mesh_section_full/rsift_mesh_section_bytes)。intern ヒット率計測は InternPool の ID が非消費と判明したため `ShapeMeter` (FoldBuildHasher) に置換 = 「shape-cache hit率」footer の実消費 (RD24 99.9% / 36c 90.9%)。**RD24 §5.1 合計 C 12.80 s < B 13.93 s で初の逆転勝利** (頂点 19.44M vs 49.26M (−60.5%)、bytes −76.3%、ACMR 1.030 も勝)。36c では 229 vs 213 ms で依然負け = 誠実併記 (交差点は規模依存、BENCH §7-7)
- **HM-2 (ソート解体再設計 = 36c S1 25.2→5.94 ms/S2 21.4→5.35 ms、RD24 S1 445-450 ms/S2 422-436 ms、両スケールで時間・精度とも B 勝利)**: saga を全機械記録 — (i) stage 分解 (一時計測、撤収済) で旧 96 B タプル units sort 単体 = RD24 で 1.41 s (全コスト 74%) を確定 (ii) Rec 48B+par_sort 808 ms/SecPlan 借用 753 ms (iii) **u128 ident32 版は誤順指紋不一致 (36c S1 15,915→10,844+デノミ変化) で事前宣言規約どおり撤回** (iv) (hi|lo) 生詰め版は desc ソートへの昇順フィールド混在という kind/skey 順序反転バグを指紋が検出 → 補数格納 (1-kind, 0xFF_FF_FF−skey) で根治・bit 一致 (v) nproc=2 (2 vCPU) の発見 → rayon par_sort の 264 MB マージバッファは逆効率、MSD radix も ±0 (vi) **決定的根拠: RD24 で A (重心 Z のみ) の誤順 3.66%/7.27% ≈ 旧 C の 5 フィールド全順序 3.59%/7.64%** → 旧 C の高コスト全順序は精度を買えていなかった = vanity complexity と機械確定 (vii) 最終解: C 公式順序を **(depth 降順, kind 補数, 追番) の単一 u64 キー** (pack = depth<<32 | (1-kind)<<31 | seq)、16 B レコード `{key, payload=(sec<<32)|qi}`、直列 `sort_unstable`、gather は cursor 直書き (out 往復廃止)。結果: 36c 時間 4.2x/4.0x 改善かつ **B (10.4/9.3 ms) を初めて時間で下回る**、誤順も S2 5.19→4.50% 改善。RD24: S1 C 441.7 ms/3.79% vs B 553.9/7.79%、S2 C 421.7/2.89% vs B 515.3/19.68%
- **HM-3 (可視性 graph 採用 = §8「本丸」backlog の解決)**: 旧 C DDA レイは RD24 で時間 60x 超 + 過剰可視 12x (S1: 101 可視 @1.11 ms vs B graph 8 可視 @18.3 µs) と確定 → 解体し **A/B と同一の SQ 共有実装 `sq_graph_find_visible` (cam/planes/96.0/近傍 ON 同一引数) の遅延構築呼出**に転換 = C の可視数・時間は B と定義上 bit 同値。健全性 pin 常駐: 新 C の可視数が旧 DDA を超過したら panic (`HM-3: graph 採用 C が DDA 旧経路より過剰可視 = 設計破綻`)。§5.1 の C 備考も「不要 (独自 DDA 経路)」→「graph 遅延採用 (§5.4 HM-3 で課金)」へ整合修正
- 検証 (全機械): 両バイナリビルド警告 0、36c/RD24 両 run で HM-3 assert 通過、構造値全項目 bit 一致 (頂点 49,257,208/19,442,804・bytes 1,576,230,656/985,144,160/233,313,648・可視 8/11・誤順カウント全列)。編集差分 parity 136/136 (36c)・788/788 (RD24) 不一致 0 維持、reopts/ACMR 不変 (36c 1.049 vs 0.858 / RD24 1.401 vs 1.004 = wave 215 誠実記録の継続)。合成: 36c C 28.11 ms = 1.25x vs A (初の両者明確超過)、RD24 C 23.78 ms = 1.48x vs A
- 誠実注記 (隠蔽なし・全て BENCH §7/§9.3 にも転記): (a) 36c S1 合併誤順は +0.56 pp tie-luck 悪化 (7.53→8.09%、判定不変) (b) 36c メッシュ合計は依然 C 負け (229 vs 213 ms = 規模交差) (c) 実体カリング RD24 は 600 体が 784² に希薄化するシナリオ歪みで参考級 (B 4 描画/C 0) — 正規比較は 36c (d) I/O 圧縮率は B 勝 (0.038 vs 0.084 = 速度優先設計の既知 trade-off) (e) 差分 ACMR 悪化は継続課題 (§8 backlog)
- 誠実境界: bench.yml への RD24 ゲート配線は workflow ファイルが AI トークンの権限外 (bench.yml 冒頭の設置注記どおり) のため本 wave では未実施 — 提案 diff は wave 報告で提示。本 wave の変更は bench ハーネス (examples) + diff_mesh.rs の括弧警告除去のみ = opt-gfx lib 本体は不変 (intern_pool.rs への consumer ゼロ API 追加は検討後に `git checkout` で撤回済 = stub/未消費 API 禁止規律)

## wave 217 HN (2026-08-03) — RD24 シナリオの機械スケーリング (実体 40,016 体/編集 7,980 回) + diff-slot OOM 根治 + 「RD24 実体歪み」誠実境界の解消

- 背景: ユーザー指示 (逐語)「Sodiumの大量の項目（2000項目前後）の実測値と全く同じ内容でRsGraphicsのベンチを回して最低限Sodium以上に」+「チェックするテストは殆ど変更不可」「生成アルゴリズム・判定・トリガーは一切変えず count/surface/tick/ワールドサイズ/カメラ・実体密度を機械的に拡大」。wave 216 HM-3 の誠実境界 (RD24 実体章は 600 体希薄化の参考級) が残存 = 密度が機械拡大されていなかった。**36c 基準密度の面積比例規約 `base x AREA_CHUNKS/36` (usize floor 除算) を採用** (36c: AREA=36 で厳密不変 — 回帰ネットで bit 一致を機械確認)。実体 600 → **40,016 体** (600x2,401/36)、編集 120 → **7,980 回** (120x2,401/36 = 8,003.33 を 60 frame の倍数に floor 丸め = 133 件x60 frame — 3 pipe が独立に同列を replay する整合制約のため `% EDIT_FRAMES == 0` を構造強制)。フレーム時間軸 (60 frame) は面積非依存で固定
- 根治 (1) — **差分レーンの疑似拡張破綻**: `edit_sim_diff` の warmup/timed ループが `edits[fr*2]/[fr*2+1]` ハードコード (36c 前提) で、RD24 では先頭 120 件のみ消費 = 本 wave 目的を静かに裏切る設計破綻。`diff_fq = N_EDITS_PIPE/EDIT_FRAMES` の機械配分に根治 (36c では fq=2 で従来と厳密一致)
- 根治 (2) — **diff-slot OOM (exit 137)**: スケーリング後 RD24 で差分レーンが OOM 死 (touched 9,425 セクション x `vec![0u32; 17^3x27]` 0.53MB ≈ 5GB > 3GB sandbox)。slot key 空間を **(pos, 法線 27 組合せ) → (pos, 軸 6 法線 dir)** に縮約 (DIFF_FACES は軸 6 のみ使用 = 21/27 は永久未使用の構造的無駄。pos+dir 単射性は保たれ意味論不変)。0.53MB → 0.118MB/セクション (4.5x 削減)。lib テスト **1,492/1,492**・36c 出力 (ACMR 1.049/0.858・parity 136・reopts 38・face_evals 4,986) bit 一致で非回帰確認
- 副産物 (正直記録): 差分 ACMR 参照の slot 直接再構成を試作 → ACMR が 0.858→2.006 と乖離 (発行順・dense ID 規約の位相差) で**撤回し FULL rescan 参照に復帰**。教訓: 参照系置換は「同一指標が出ること」の事前機械確認が先
- ~~次 wave 直結の発見: `wide_static_bench` 走査で本番 wiring 未接続 2 件 (RsSliceFillGuard/CULLIANCE_NULL_BLOCK)~~ → **撤回 (2026-08-03 機械再検証)**: 環境リセット後の出荷前検証で全ツリー走査 (grep: シンボル・`wiring.rs`・`sec_wrap.rs`・`cfg(any())`・Guard 構造体・0xAB フィル) を実施したところ、引用ファイル・シンボルは **repo に一切存在しない** (一時スクラッチ状態か誤記録のいずれか。「機械発見」と記した検証自体が再現不能 = 監査規律違反)。本項は wave 218 backlog から抹消し、教訓として検証不能な引用の記録禁止を再確認する
- 最終機械値 (RD24・複数独立 run で構造値不変): 実体 **B (錐台) 601 描画 @173 µs vs C (3,000 レイ真遮蔽) 167 描画 @1,030 µs** (occluded 39,849 / far+fov 39,415) = 時間 B 勝・深度 C 勝の設計差が有意に (旧歪み版 B 4/C 0)。編集 7,980 / 汚染延べ 53,841: **A 4.644s ← 最速 / C 差分 5.717s (bytes A の 1.1%・B の 1.8%) / B 16.812s / C 全量 36.013s**、パリティ **9,425/9,425 不一致 0**、reopts=2,941 (高密度帯で閾値発火 = A 最速の理由、§8 backlog 登録)、差分 ACMR 1.045 vs 0.889 (+0.156 誠実記録継続)、face_evals 334,884。総合判定 (BENCH §9.2) **C ≥ B 全項目維持** (I/O 圧縮率・実体時間の 2 例外は誠実併記継続)

## wave 214 HK (2026-08-02/03) — 実体カリング V2 統一 (2.57 → 0.53 ms、4.9x) + ChunkBucketGate 撤去 (0 レイで「永久可視」を保持する虚偽可視バグ + 純損デッドコード) + banner「FastEntityCuller V2」の初の真実化

- 背景: ユーザー指示「偏屈なシニアグラフィックスアーキテクトとして車輪の再発明を批判し最適を追求」の棚卸しで 2 件を機械発見。(a) bench C 行が legacy `EntityCuller` (27 レイ) を消費し B の 209 倍低速 (2.57 ms vs 12.3 µs) (b) `dx12_engine` boot banner は "FastEntityCuller V2" を CPU feature 宣言、しかし本番 wiring (`full_graph_wiring` tick_world) は legacy `EntityCuller` を駆動 — **宣言≠実装の矛盾** (V2 の唯一の実消費者は example)。2 基の culler 並存は車輪の再発明そのもの
- 原因特定 (V2 も正義ではなかった): 密度スイープ bench 実験で V2 の `chunk_skip=287/600` を機械検出 → 読解で確定: ChunkBucketGate の skip は `last_eval_tick` を更新しないため、(i) 新規スロット (既定 visible=true) は **0 レイのまま永久可視** (全セル不透明世界でも — 旧 wave 88 CL-3 pin がこのバグを「点滅防止の仕様」として固定していた = pin がバグを法治化) (ii) gate 条件が真であり続けるスロットは due に到達しても評価自体を永久免除 (変異でなく構造欠陥)。しかも skip 判定自体が opaque 3 probe + 1 レイを毎 tick 要求し skip 節約 (5 レイ/10 tick amortize) を恒に上回る**純損デッドコード**。かつ `skipped_far` カウンタを膨張 (真 far は 178 のところ 421 と誤表示)
- 根治: (a) **ゲート全撤去** — 点滅防止の語彙は due 幅ヒステリシス (10 tick 前回値保持) が既に担っており不要。`chunk_cull_gate` フィールド・`last_chunk_skipped`・probe/ray ブロー・カウンタ合流を全削除 (フィールドは外部消費者ゼロを確認)。出力は虚偽可視分のみ変化 (b) CL-3 pin を「due 幅内の前回値保持 = 正統ヒステリシス」へ改訂 + 新 RED ピン「完全埋込実体は due 実評価で不可視収束」(c) **V2 エンジン本採用の機械根拠 (密度膝点)**: dense 5³=125 レイ真値計測器に対し 5/9/27 レイは pops 4/3/2 件のみ (waste 全員 0 = 保守側) — 27 レイの 3.9x コストが買うのは approximal 2 件のみ = 膝点は 5 レイ帯。27 レイの legacy も真値対し 2 件欠落する近似級で、dense との差は「誤差」であって「欠陥」ではないと機械判定 (d) bench C 行を V2 へ (e) 本番 wiring を V2 へ + **CL-4 将来課題を解消** (実カメラ FOV の対角半角余弦 + 0.15 rad 余裕を毎フレーム供給)
- 却下記録: セクション可視性への V2 転用は機械却下 — 16³ 大 AABB に 5 レイは粗すぎ pops 大量 (S2 15→3 = 描画総崩れのポップ) かつ過剰可視残留 (S1: graph 7 / legacy 61 / V2 39)。セクション遮蔽の本丸 (horizon/graph 系設計転換) は BENCH doc §8 大物 backlog に登録
- 検証 (全機械): RED×2 新規 (完全埋込収束 + due 幅ヒステリシス) → GREEN → opt-gfx **1486/1486** 全緑 (+1 = pin 2 追加 − 1 改訂)。adversarial 実効変異 (due カレンダ `>=`→`>`) = **RED×4** (新 pin 2 + CL-2/CL-4 既存 pin 2 が defendant として機能) + MD5-VERIFIED×2 ファイル。pseudo_mc_bench 再計測 (同一ワールド同一プロセス、環境復元後の再現走でも可視集合/レイ数 bit 同値): C 実体カリング **2.572 ms → 0.529 ms (4.9x)**・rays 10,500 → 2,110 (−80%)・**V2 可視 ⊂ legacy 可視 (v2_only=0)**・個別カウント非干渉 (半透明誤順/セクション集合/draw call は wave 213 と bit 同一 = 決定性)。合成「重い 1 フレーム」33.34 ms vs A 35.11 ms = **1.00-1.05x の同値圏** (A の run 間ノイズ ±3-5% を誠実に併記、成分値のみ厳密)
- 誠実境界: V2 の pops 4 件 (0.9%) は近似誤差の honest な代価として docs 記録 (tr7zw 系 modと同級の設計帯)。wiring 側の targets は emissive 光源 AABB の代替実体で結果は消費者なしで破棄される既存 §7「評価実効・消費なし」パターンは不変 (banner 真実化が目的で拠実体接続は将来課題)。bench 「C 行が FOV を持つ」語彙変更により可視集合は 175→125 と縮小 (内訳: FOV 整合 −48、サンプリ −2 — legacy 行値は wave 213 凍結で参照可能)。(環境リセットで消失した初出セッションの内容を決定的再現 — 両環境の MD5 が一致)
...
- 根治: `restore_official_mods_search(mod_dir, source_dirs)` 新設 = 優先度順 (rsift home → バニラ `.minecraft/mods` → バニラ `versions/rsift-1.21.11`) に**公式 mod が見つかった最初の源からのみ**コピー (探索打切)。`vanilla_minecraft_dir()` を新設 (cfg 別既定パス: Windows=APPDATA/.minecraft、macOS=~/Library/Application Support/minecraft、Linux=~/.minecraft)。単一源 `restore_official_mods` の全保証を継承 (固定 3 名のみ・既存上書き絶対禁止・marker で削除意思を恒久尊重・modsec 非迂回) + `OfficialModRestore.source_dir` で「どの源から拾ったか」の監査情報を追加 (agent ログに出る)。mod_dir 確定状態 (marker 残存/全本揃い) は源に依らず打切
- 検証 (全機械): unit +7 (第2源フォールバック+source_dir 報告/第1源優先打切/全源空/marker で探索停止/源==mods 防御でも後続源へ進行/探索源順 pin/vanilla パス形状 pin)。adversarial 変異 ×2 (continue→break でフォールバック消失=RED×2 / marker 短絡除去=RED) + MD5-VERIFIED×2。実 JVM ハーネス D/E: D=偽バニラ HOME (`.minecraft/mods` に公式 3 本) + natives 無公式 → **`official mods auto-restored from /tmp/hg-home/.minecraft/mods: [3 名]`** を機械観測 (= ユーザー実機形態と同型の救済経路が実動作)、E=全源空 → 「no official mods in any source dir」+ 日本語ガイド行 + exit=0。wave 209/210 両保全 (F3 GREEN `HB_F3_LINES=[base line, RsGraphics Render (Rsift)]`)・fmt 自己起因 0・簡体字 0
- 誠実境界: バニラ側に公式 mod が残っているかは実機依存 (ユーザーが消していればフォールバックも失敗 → ガイド行が残る設計)。復旧を行わない安全側設計は wave 210 と同一 (確定不能時は何もしない + 理由を 1 行ログ)。stale な旧ビルド公式 dll を拾った場合の ABI 適合は abi_stable テーブルの版協議に委譲 (wave 既存機構、無検討の迂回ではない)

## wave 210 HF (2026-08-02) — 実機 #4/#5「mods loaded OK: []」(mods フォルダ空) の根治 = 公式 Mod 自動復旧 (least-privilege) + Prism 配備ログ対称化 + 空 mods 日本語ガイド

- 実機一次解析 (launch ログ repo 同梱分): #4 game log = `mod_dir from game cwd: ...\PrismLauncher\instances\rsift\.minecraft\mods` + `mods loaded OK: []` (フォルダは存在するが platform ext 0 件)。#2 setup log = `exit_code=0` + scan 5 lib 全 OK + **launcher flow は `mods=["rsgraphics.dll","rsreplay.dll","rszoom.dll"]` と記録される一方、prism flow はインスタンス登録行のみで mods 配備結果行が存在しない**。即ち本来 3 本配備されるはずの `<inst>/.minecraft/mods` が空で、かつその事実がログから追えない構造だった
- 真の root cause 選定 (推測排除): (a) mods 配備コード自体は launcher/prism 双方に wave 199/202 から存在 (`git log -S` で立証)、bundle にも 3 mod cdylib が同梱済 (DIAG-Windows.txt / selftest-win 一覧で機械確認) — つまり「配備するはずが、実機では空」であり、コピー失敗・ユーザー操作・環境要因のどれかはログ欠落で原理的に確定不能。(b) 確定可能な構造欠陥は 2 点: **prism ステップだけ mods/natives 配備結果を log しない対称性欠陥** (launcher 側は `version_dir=... natives={:?} mods={:?}` 行あり) と、**空 mods でも `mods loaded OK: []` で沈黙完了する診断性欠陥**。(c) 上記を踏まえ、原因確定に依存しない根治方針 = **ゲーム起動時にエージェント自身が自壊復旧する** (rsift home = natives には公式 mod が同一バイトで存在するため、setup 配備成否に依らず収束させる)
- 根治 3 点: (a) **rsift-api `restore_official_mods(mod_dir, home_dir)`** — コピー元は固定 3 名のみ (`official_mod_file_names` = setup の LIB_* と名称 pin で厳密一致)。権限境界: 任意ファイル/path traversal の余地ゼロ・**既存ファイルは絶対に上書きしない** (同名既存はその本のみスキップ)・自動復旧は `.rsift-official-mods-restored` marker に監査情報 (source/restored/unix_time) 付きで記録し **marker が残る限り二度と実行しない = ユーザーが復旧後に公式 mod を消した意思を恒久的に尊重**・復旧後も通常経路 (modsec vetting) でロードするためゲート迂回なし。(b) agent_premain を **存在検査より前** に候補列挙ログ (`mod candidates (N): [...]`) → restore 試行 (結果/理由を必ず 1 行) の順に配線 = mods dir 自体が消えた実機でも自壊復旧。ロード 0 件かつフォルダ空継続時のみ日本語ガイド行 (rsift-setup 再実行 or mod 配置を案内 — ファイル有りで 0 件なら modsec/リンカ由来の個別エラーが既に残るので誤誘導防止で出さない)。(c) rsift-setup に `prism_summary_line` を新設し prism ステップへ `instance_dir=... natives={:?} mods={:?}` 行を追加 (launcher 側と対称、mods=[] でも省略せず出力)
- 検証 (全機械): unit — rsift-api +8 (名称 pin/空から全復旧+バイト一致+marker 監査/欠損分のみ補充・既存上書き禁止/marker 尊重/home 無公式時 skip/home==mods 防御/固定名以外不コピー/冪等)、rsift-setup +1 (prism_summary_line が mods=[...] を必ず含み空でも mods=[] と出る pin)。adversarial 変異 ×2: 上書きガード除去→RED (fills_missing 系)/marker 検査除去→RED×2 (尊重・冪等) + 両方 MD5-VERIFIED 復元。実 JVM ハーネス (wave 209 の 2-loader 同等環境) A/B/C: A=空 mods・home 無公式→`mod candidates (0): []` + `official mod restore: no official mods in home dir` + 日本語ガイド行、B=home に公式 3 本→`official mods auto-restored ... [3 名]` + marker 内容機械確認 + ガベージ dll は modsec/リンカ拒否 (loaded 0 = 正しい拒否)、C=marker 残存+実ファイル消去→`restore marker present — user intent respected` で再コピー無し。**全シナリオ exit=0 かつ wave 209 の F3 GREEN (`HB_F3_LINES=[base line, RsGraphics Render (Rsift)]`) 維持**・CNFE 3 行のみ・fmt 自己起因 0・簡体字 0・seal 6 ゲート全 PASS
- 誠実境界: ユーザーの mods フォルダが空だった**個別原因はログ欠落で確定不能** (本 wave は原因確定に依存せず系を自己修復化 + 以降の #6 ログから配備成否が機械判別可能になる)。restore のコピー元は rsift home (natives) に公式 mod が**存在する場合のみ**有効 — ユーザーの natives に 3 dll が無い場合は skip 理由 + ガイド行が残るため #6 ログで次の一手が一意に読める設計。mods フォルダにユーザー自作 mod が居ても同名でなければ干渉しない (least-privilege の上書き禁止は変異検定で機械固定)

## wave 209 HE (2026-08-02) — 実機 #5「block ready 後も全 vanilla クラス CNFE 永続 (135 秒 800 行超)」の根治 = ClassLoad jcache (ローダー非依存) + f3 spec 先行登録 (RefTrans 非依存化) + CNFE throttle

- 実機 #5 一次解析 (rsift-bootstrap.log、wave 208 版稼働を機械確定): 改善点 = ブリッジ群の準備が完了 (RsiftScreenHooks ready / RsiftModBridge/RsiftPlatformBridge loaded / UiBridge=true ModBridge=true / RetransformClasses rc=0 で wave 208 根治の実機証明も同時に取得)。残存欠陥 = `loadClass(net.minecraft.client.Minecraft)` を含む **全 vanilla クラスが捕捉ローダーから永遠に CNFE**。game cwd の mods に dll が無い = `mods loaded OK: []` は正しい動作 (欠陥ではない)
- 真の root cause (機械証拠): (1) JNI FindClass (system loader 経路) も CNFE = **vanilla 本体が system loader に居ない** (= Prism のラッパー起動で子ローダー分離された型。実機 #5: CFLHが最初に見た net/minecraft 系は net/minecraft/client/main/Main ではなく ClientBrandRetriever t+11.6s = ラッパー経路特徴) (2) CFLH first-capture ローダーでも Minecraft 系を解決不能 (prod #5 で実証) (3) f3_marker は「反射特定 → その瞬間の強制ロードで未パッチ確定 → RefTrans 必須」の構造で、さらに post_bridge_boot 時点の登録でも対象クラスが先に define されれば CFLH 注入が間に合わない = **構造的に RefTrans + タイミングに弱点**
- 根治 (3 点): (a) **ClassLoad イベント (num=55=CFLH 連番/slot5、Name 引数無しのため GetClassSignature num=48)** で重要 6 クラス (Minecraft/Screen/DebugScreenOverlay×2/DebugScreenEntryList/Connection) の jclass を global ref jcache 化 → find_minecraft_class が jcache 最優先 = **どのローダーが定義しても必ず届く** (b) f3 静的 spec (1.21.11 確定 getLines=`()Ljava/util/List;`/String) を **CFLH install 成功直後** に冪等先行登録 = 対象クラス自然ロード時に CFLH が TAIL 注入 = **RefTrans に一切依存しない**。wave 208 で「permission boundary」誤判断により見送ったのは rsift-parser 側テーブル編集失敗の誤帰因と判明 (register API 自体は公開口) (c) CNFE dump に同名 2 秒 1 行 throttle (実機 800+ 行スパムの根治 = 診断性は保持)
- 検証 (全機械): 2-loader ハーネス (Prism wrapper 型: loader A=ClientBrandRetriever のみ / loader B=Minecraft 等 = javac 不在のため手組 class ライタで fake 4 クラス emit) — **RED** (修正前 .so): HB_F3_LINES=[base line] (=マーカー無し) + Minecraft CNFE 89 行 + f3 skipped honestly → **GREEN** (修正後 .so): **HB_F3_LINES=[base line, RsGraphics Render (Rsift)]** (ユーザーの「F3 に RsGraphics Render」要望の実現形を実機同等環境で実証) + wanted class captured ×2 + CNFE=3 (load 前レースのみ) + exit=0。unit 19 (ClassLoad 55/slot5/GetClassSignature 47 pin + wanted 狙撃 pin + prereg exact-spec pin 追加) ・変異 RED×2 (spec メソッド rename / wanted クラス rename) + MD5-VERIFIED×2。fmt 自己起因 0・seal 6 ゲート全 PASS
- 誠実境界: GetClassSignature num=48 は Allocate(46)/Deallocate(47) 連番 + ハーネス機械検証 (ガベージならセットアップ失敗/不一致で顕在) の 2 点で確定。実機の append 注入は OnLoad 相 AddToBootstrapClassLoaderSearch (wave 207 実証済) 前提 = 旧 bundle 利用中のユーザは再取得が必須。Screen 系のフック/タイトル改変も同様に ClassLoad jcache 経路成立後の動作 (実機の次回ログで確定)

## wave 208 HD (2026-08-02) — 実機 rc=99 (RetransformClasses MUST_POSSESS_CAPABILITY) の根治候補化 + 降格経路の機械検証 + 権限境界調査

- 状況 (正直な復元記録): 先行セッションの未 commit 作業 (sweep 調査/Metro AOT 仮説等) は環境リセットで作業木と共に消失。一次情報 (OpenJDK 実装) から再構築、以後は wave 中コミットを早期化する運用に修正。調査で確定: HA-4 (wave 206) で jvmtiEnv 共有キャッシュは HEAD に実装済みで、HEAD の retransform 経路 (`jvmti_events::retransform_classes`) は全て共有 env 経由 = caps 保有 env が使われる構造。実機の rc=99 が HEAD 系と別 build (未発布 bundle) のログ由来である以上、**真因は「呼出 env が caps を持たない経路の存在」か「VM 自体の付与拒否」の 2 択に絞るための検査経路の内蔵が本 wave の本質** (推測で断定しない)
- 一次情報確定: (1) JVMTI capability は env オブジェクト単位管理 (OpenJDK jvmti.xml: 「The state of a environment includes: the event callbacks / the set of events which are enabled / the capabilities」と明記、加えて HotSpot `JvmtiExport::get_jvmti_interface` が呼出ごとに `create_a_jvmti(version)` を new = HA-4 と整合) (2) rc=99 の発生源は `JvmtiEnv::RetransformClasses` 冒頭の `get_capabilities()->can_retransform_classes()` 早期 return (OpenJDK master jvmtiEnv.cpp 機械確認 — 他経路に 99 の return は存在しない) (3) AddCapabilities/SuspendAllVirtualThreads 系は `JvmtiSuspendControl::print()` 経由の延長戦という補助文脈のみ — 請求自体は additive で VM が拒否可能 (rc≠0)
- 根治実装 2 点: (a) `ensure_retransform_caps_on_call_env` (jvmti_events.rs) — retransform_classes が「実際に呼出に使う env」で必ず GetCapabilities → 未取得なら AddCapabilities {redefine, retransform, retransform_any} (additive 3 bit のみ) を試行。caps 保有済み env では早期 return で no-op (= Temurin 系 0 リスク)、不足ならその場根治、VM 拒否なら rc≠0 を `[warn] jvmti` に固定出力して実機ログを判定可能に (b) `RSIFT_DISABLE_RETRANSFORM` テストフック (既定 off) — 実 JVM ハーネスで stub rc=99 の降格経路を検証可能に。降格時の動作 = f3_marker 登録は reflection 検証済みで成立・RefTrans のみ 99 で静黙継続・JVM_EXIT=0・ゲーム無害継続 = 実機ログのシグネチャと完全一致することを Temurin 21 ハーネスで機械確認
- 見送り (権限境界): f3 marker の「load-free 静的 spec 先行登録」は設計上破棄 — rsift-parser の spec レジストリは patcher.rs の first-match 権限テーブル (`is_targetable_external` 狙撃テスト) と連動しており、reflection 真名検証前の登録は権限境界を広げる (変異 RED で狙撃検知を確認)。登録は従来通り reflection 検証済みのみとし、never-widens pin テストで機械固定。RefTrans 不能 VM + 対象クラス既ロードの場合 f3/title が適用不能なのは JVMTI 設計由来の正直な制約 (loaded class の bytecode 変更は Redefine/Retransform のみ・再取得不能) として台帳記録
- 検証: unit 18 (jvm: 既定 stub off pin + caps additive 3bit pin + 権限境界 pin 追加で 15→18)・parser 19・setup 39・rszoom 9・api 82・opt-gfx 1485 全緑。実 JVM ハーネス 2 モード: 既定 = RetransformClasses rc=0 ×2 + f3 registered + F3_MARKER_OK (回帰なし)、stub = rc=99 降格で JVM_EXIT=0・クラッシュなし・CFLH 継続 (ユーザーマシンの症状そのものを制御下で再現)。adversarial 変異 RED×2 (registry 汚染変異 → 権限 pin RED / env ガード反転 → 既定 pin RED) + MD5-VERIFIED×2。fmt 自己起因 0・seal 6 ゲート全 PASS
- 誠実境界: 実機で rc=99 が再発しないことの証明は新版同梱ビルドでの次回起動ログ (AddCapabilities rc 有無) が唯一の検定口 = caps 不足なら根治で消え、VM 拒否なら警告行が出る。いずれにせよ「推測なしに原因特定できる構造」を実機へ届けるのが本 wave の提供価値

## wave 207 HC (2026-08-02) — 実機 #4「まだvanilla判定」の根治 = bootstrap jar が JDK 24/25 ビルド (major 68/69) で MC Java 21 では全 consumer UCVE + RsiftHooks リンク不能 + live 相 AddToBootstrap WRONG_PHASE

- ユーザー実機報告 (逐語): 「#4追加済み、まだvanila判定。」— logs/#4/rsift-boostrap.log (4327 行・実機) を一次解析
- #4 機械確定事実: wave 206 修復版稼働確認 (t+ セッション相対・CFLH install attached=1・AddCapabilities rc=0・capture 成功 t+4377ms・FOUND attempt 3)。しかし (a) `mods loaded OK: []` (b) `screen_hooks — load failed: load ScreenHooks: JavaException` + `URL class: JavaException` が **188 秒間 4 秒周期で 3000+ 行**連発 (c) `retransform — no statically targeted classes loaded yet` (d) `f3_marker — no F3-lines method found` (e) title_marker 行一切無し。ゲーム本体は正常起動 = agent クラッシュ解消は実機確認済
- 解析 (全て機械値): jar バージョン一次確認 `od -tx1 -N8` → **収録 18 クラスの major が 3 件 68 / 15 件 69** (= JDK 24/25 の javac 出力、prebuilt 由来)。MC 1.21.11 の Java は 21 (major 65 上限)→ URLClassLoader 経由の全 consumer が `UnsupportedClassVersionError` を throw → wave 204-206 の全 bootstrap 経路 (ScreenHooks/UiBridge/ModBridge/PlatformBridge/ClassTransformer/retransform sweep) が全滅 #4 の構造と完全一致。加えて (i) HEAD 注入の invokestatic 先 com/rsift/RsiftHooks はゲーム loader から解決不能 (sandbox 旧 harness が bootstrap jar を -cp 同梱していたため **wave 206 で見逃した検証盲**)、(ii) Temurin 21 実測で live 相の AddToBootstrapClassLoaderSearch が JVMTI_ERROR_WRONG_PHASE (112) を返す
- 根治 (4 点): **(1) prebuilt jar の major を 68/69→65 に機械パッチ** (python zipfile 書換・全 18 クラス・od 検証 0x41)。class 内容は変更なし (バージョンヘッダのみ) — 内容は元々完全バイト列のため安全。**(2) Agent_OnLoad (onload 相) で AddToBootstrapClassLoaderSearch (num=149 → C index 148)** を呼出 (jvmti.xml 一次情報: onload 相では任意 segment 受理、live 相では JAR 限定 + build 差で WRONG_PHASE 実測)。rc 0 必携ログ + live 相フォールバック (install 後) は残留。**(3) `dump_pending_exception` 新設**: exception_occurred → exception_clear → toString 要約 → env.throw で pending 復元の 4 段。「pending ままの getMessage は投げられる」という JNI 規約で `<dump failed>` 連発だった旧診断を根治し、全 JavaException 経路 (screen_hooks/URL/ModBridge/PlatformBridge/retransform sweep/loadClass) で 1 行 class+message を必ず残す。**(4) agent_opts::dll_directory を Linux でも dladdr 実装** (元 Windows 限定 GetModuleFileNameW 同型) — sandbox の .so でも natives 同型の jar 可視経路を構築可能にし、「bootstrap jar を -cp に入れない」実機同型 harness を有効化
- 実 JVM RED→GREEN (Temurin 21.0.8+9、MC Java 21 相当): RED = NoClassDefFoundError: com/rsift/RsiftHooks + UCVE 全 consumer × 3000 行相当再現。GREEN = `Agent_OnLoad: AddToBootstrapClassLoaderSearch rc=0` / `RsiftScreenHooks ready` / `RsiftModBridge loaded` / `RsiftPlatformBridge loaded` / `UiBridge=true ModBridge=true` / `RetransformClasses rc=0` ×2 / `f3_marker — registered on …#getLines (String)` + `W206 F3_MARKER_OK` / `title_marker — window title set: Minecraft* 1.21.11 - Rsift (RsGraphics Render)` / LINKED×9 / **JVM_EXIT=0** 全 PASS。「jar を -cp に入れない」実機同型で classpath 経由は全遮断した上での証明
- 併せて確定した設計知見: (i) ブリッジ jar は **bootstrap CL 経由の NR 解決** を最優先で担保する必要 (AddToBootstrap 経路) (ii) JavaException の一律 map_err はメッセージ欠落を伴いやすい (dump 復元が必須) (iii) prebuilt jar はコンパイル環境の javac version に data-dependent で脆弱 → バンドル時に major 確認を診断化 (DIAG 化今後)
- 検定: jvm 15・parser 19・setup 39・rszoom 9・api 82・opt-gfx 1485 全緑。fmt 自己起因 0・簡体字 0 (seal ゲート 1 san 正典)。seal 6 ゲート全 PASS (digest 004c1cf5fb17bfe8)
- 誠実境界: (i) prebuilt jar の「内容」が完全である前提は変わらず、major 65 書換のみ — 本質的には「JDK 21 ターゲットで javac 再ビルド」が筋であり、次回の jar 再生成時は -target 21 を運用ルール化すべき (ii) Windows/arm Linux では dladdr 位置と実機 natives 配置一致の再現は未実施 (iii) #5 (新版稼働) 実機ログで UiBridge=true + F3/タイトル出現の最終確認が要 (iv) mods loaded OK: [] は mod 側別欠陥 (Mod nodule 0 件) で本 HC-1 の vanilla 判定とは別系統 — 残課題

## wave 206 HB (2026-08-02) — 実 JVM (jdk4py/Temurin 25) 実機検証で判明した構造的欠陥 3 件根治 (SIGSEGV/rc=98/rc=99)

- 対象: rsift-jvm (agent_bridge/jvmti_events/agent_log)、ci/TRIGGER.md、本書
- ユーザー指示 (逐語): 「jdk4pyでJDKを取得」— wave 205 の誠実制約「sandbox に Java 無し = 実 JVM 検証不可」を打破するため、jdk4py (Temurin 25.0.2+10 JRE) を導入し、取得 JVM に `librsift_jvm.so` を `-agentpath` でロードして CFLH/捕捉/Retransform/F3/タイトル/verifier 通過を機械検証する wave
- 検証ハーネス: javac 不在 (jdk4py は JRE のみ・Maven/Adoptium raw 取得は sandbox 遮断) のため python 手組みクラスライタで 10 クラス生成 (Minecraft 1.21.11 mojmap と同じ内部名: `net/minecraft/client/Minecraft`・`Screen#init`・`Connection#channelRead0`・`RenderSystem#flipFrame`・`ServerLevel#tick`・`DebugScreenEntryList#getLines` (Signature List\<String\> + おとり `getEntries` List\<DebugScreenEntry\>)・`Window#setTitle` 他)。生成直後の fixture 誤りを **JVM verifier が 6 回検出** (opcode/label/stackmap) = 独立監査として機能。エージェント無しスモーク全緑を先に確立 (9 クラス LINKED・全メソッド実行 OK)
- **HA-2 [大 = JVM ごとクラッシュ・ユーザー実機にも到達確定していた]**: wave 205 の「CFLH install を attach 前 (最速) へ前倒し」は、**未アタッチのネイティブスレッドから `GetEnv(JVMTI)` を呼ぶと JVM ごと SIGSEGV** する設計だった。実機観測: hs_err `SIGSEGV at JvmtiExport::get_jvmti_interface+0xd0 ← jni_GetEnv+0x85`、si_addr=0x52c (=null+offset)。HotSpot 一次情報: LIVE 相の `get_jvmti_interface` は `ThreadLocalStorage::thread()` (=未アタッチでは null) に対して `ThreadInVMfromNative` を構築するため null デリファレンスは仕様上の行為。根治: install を attach ループ「成功直後・loader 探索前」へ移動 (HOOK_INSTALLED 冪等で先着1回)。捕捉鮮度 (=wave 205 欠陥B の意図) は install→loader 探索の順序で維持。**影響: wave 205 同梱の setup-v1 ビルドはユーザー環境でも起動 ~1 秒後にゲーム JVM ごとクラッシュする見込み (RED で確認済) — wave 206 で封じ**
- **HA-3 [中 = retransform 経路の全滅連鎖]**: 1 つでも付与不能な capability を含めると `AddCapabilities` は JVMTI_ERROR_NOT_AVAILABLE (98) で**何も付与しない**。実機観測: live 相から early 系 bit 42 を含む 1 回請求 → rc=98 → 全 5 capability bit が false → RetransformClasses も rc=99 に連鎖。根治: `GetPotentialCapabilities` (idx 139) で取得可能集合を引き `desired ∩ potential` のみ請求 (early 系は OnLoad 相でしか取れない JVM では自動除外) + 部分失敗は warn して CFLH install は継続 (rc ログで診断可能)。実機結果: request all_hook=true early_hook=false redefine/retransform/any=true → rc=0
- **HA-4 [大 = wave 205 の Retransform が用途を問わず永遠に動かなかった]**: **`GetEnv(JVMTI)` は呼出ごとに新しい JvmtiEnv を生成する**。HotSpot 一次情報: `get_jvmti_interface` は LIVE 相で `JvmtiEnv::create_a_jvmti(version)` を毎回 new して返す。capability は env 単位 → install 時の env#A に AddCapabilities しても、後続呼出で取り直した env#B は capability 未保有 → RetransformClasses が `MUST_POSSESS_CAPABILITY (rc=99)` で永遠に失敗する。自前ログの GetCapabilities=true は同 env 内の自己申告でしかなかった (Add/Get は対称なので bit ずれ検知不能)。根治: 先着1個の jvmtiEnv を `JVMTI_ENV` static にキャッシュし全 JVMTI 呼出 (Retransform/GetLoadedClasses/IsModifiableClass) で共有。実機結果: rc=0、CFLH 再発火でパッチ適用成功
- 副次改善: agent_log の `t+` は旧実装が epoch_ms % 1,000,000 (=16.6 分周期の下 6 桁) で時差が読めなかった → セッション相対時計 (初回ログ=0) に修正。診断 diff 読みが可能に
- 検証結果 (機械 assertion 10/10 PASS、Temurin 25.0.2+10・G1・Linux amd64): (1) JVM が `W206 END` まで生存 (SIGSEGV 解消) (2) `ClassFileLoadHook enabled (exact-spec indices)` (3) `game ClassLoader captured via CFLH` (4) `FOUND via JVMTI CFLH captured loader` (attempt 2) (5) `RetransformClasses rc=0` (5 of 11 静的対象) (6) `f3_marker registered on …DebugScreenEntryList#getLines (String)` + `RetransformClasses rc=0` (7) `W206 F3_MARKER_OK` + getLines 実出力 `[line-a, line-b, RsGraphics Render (Rsift)]` = TAIL 注入が再定義クラスの verifier を通過し実行時に観測 (8) タイトル `Minecraft* 1.21.11 - Rsift (RsGraphics Render)` setTitle 成功 + ハーネス側記録一致 (9) おとり getEntries は無傷 [] (picker が List\<DebugScreenEntry\> を拒否した証拠、CCE 無し) (10) パッチ済み 9 クラス全て LINKED (HEAD 注入バイト列が実 JVM verifier を通過)
- 検定: jvm 15・parser 19・setup 39・rszoom 9・api 82・opt-gfx 1485 全緑。本根治 3 件は JNI/JVMTI 実振る舞い変更であり unit 変異テストでは RED 化不能 — **実 JVM ハーネス自体が adversarial 機構** (修正前 = SIGSEGV・rc=98・rc=99・F3_MARKER_MISSING の RED を機械観測、修正後 = 全 assertion GREEN) として wave 内自己照査を完結
- fmt: 変更 3 ファイル (agent_bridge/jvmti_events/agent_log) は rustfmt --check 0。簡体字 0 (seal ゲート1 san 正典)
- 誠実境界: (i) ハーネスの MC クラスは実名同じ手組み fixture であり、**実 Minecraft 1.21.11 での F3/タイトル/Mods ボタン目視確認は次回のユーザー起動待ち** (rsift-bootstrap.log 送付で上記 assertion と同型の照合が可能) (ii) Windows/Temurin-21 系 (MC 同梱) でも同一 HotSpot コード経路だが別 OS/別ビルドでの再現検証は未実施 (iii) ハーネスでは頭出しキー (dll 同階層) が無いため UiBridge/ModBridge ロードは環境要因で失敗 (= ユーザー環境では natives に存在する想定。本 wave の対象外) (iv) `get_loaded_classes` 掃引戦略は本ハーネスでは未発動 (CFLH 捕捉が先に成功) — 別経路の実機検証は今後の課題
## wave 205 HA (2026-08-02) — 実機「Vanilla判定/Mods 表示なし」の構造的欠陥 4 件根治 + F3/タイトルマーカー

- 対象: rsift-jvm (jvmti_events 全書換/screen_inject/agent_bridge/agent_opts)、rsift-parser (class_rewriter/f3_marker 新設/patcher/lib)、docs/SETUP_BOOTSTRAPPER_JA.md、ci/TRIGGER.md、TRIGGER2.md
- ユーザー実機報告 (逐語): 「RsiftじゃなくてVanila判定です。#3のupload完了。しかもModsが表示されない。F3の一部にRsGraphics Renderって書いたら？」— logs/#3/rsift-boostrap.log (= ユーザー web UI upload 実機ログ) を一次解析
- 実機ログの機械確定事実: bootstrap jar `exists=true size=Some(44301)`・rsift_jvm.dll `exists=true` (wave 204 修復は実機成功)・Agent_OnLoad→deferred init→thread spawn OK まで到達。しかし (a) `agent_premain — mod_dir="…\PrismLauncher\instances\rsift\rsift-natives\mods"` → `WARN mod_dir does not exist` (= Mod ロード 0 件)、(b) `classloader_probe attempt 0..185+` が「not ready yet / all strategies failed」を連発し install_class_file_load_hook に未到達、ゲームはタイトル画面まで正常起動
- 欠陥 A [大 = Mod 全不発]: agent_opts::resolve_mod_dir が `modDir=/gameDir=` arg → opts file → `dll_dir/mods` → `./mods` の順で、**ゲームプロセスの cwd を候補に持たない**。Prism 運用では dll_dir = rsift-natives で mods は `<instance>/.minecraft/mods` (= ゲーム cwd) に存在 → natives/mods 不在で 0 件沈黙。根治: resolve_mod_dir に cwd/mods (実在時) 候補を opts file の後・dll_dir の前に挿入 + 選択由来ログ + opts の死んだ modDir も fall-through 化。pin 5 本 (serial cwd mutex)
- 欠陥 B [大 = フック 0]: classloader 探索が (i) 優先スレッド 4 名の context CL 検証 (失敗理由を残さない非自己記述) (ii) `loader_from_minecraft_get_instance` は `game_class_loader().or_else(loader_from_priority_threads)` で**構造的トートロジー** (= 独立戦略でない) のみ。1.21.11 命名は OptiFine jar srg 定数プール一次確認で mojmap 実名と反証済 (= 命名不一致が原因ではない)。根治: (1) **早期 CFLH install** (attach 前最速化) + CFLH が渡す defining loader を NewGlobalRef で直接捕捉 (CAPTURED_LOADER/captured_game_loader_raw) = 探索不要の本命、 (2) **JVMTI GetLoadedClasses 掃引戦略** (idx 77、net.minecraft.* 最初の 1 クラスの getClassLoader、10 秒周期に負荷抑制) を決定的代替として追加、 (3) 優先スレッド経路の自己記述ログ化 (見えたスレッド名一覧 + 各 thread の loader null/loadClass 例外メッセージ保存) — 「all strategies failed」のみで理由が読めなかった診断盲点を解消
- 欠陥 C [大 = 休眠手榴弾]: 旧 jvmti_events の vtable index が**全体ずれ**。正本 = OpenJDK 公式 src/hotspot/share/prims/jvmti.xml (617,807B を gh api raw で取得) の num 属性 − 1: Allocate=45 (旧 46 = off-by-one で Deallocate を 3 引数呼出し = heap 破壊相当)、Deallocate=46、GetLoadedClasses=77、IsModifiableClass=44、RetransformClasses=151、SetEventCallbacks=121、SetEventNotificationMode=1 (旧 58/75 試行)、イベント CFLH=54、**コールバック struct slot = 4** (旧 54 = イベント番号との混同)、capability bit (capabilityfield 文書順) can_redefine=9/can_generate_all_hook=26/can_retransform=37/any=38/early_hook=42、JNI GetEnv=6/NewGlobalRef=21。ユーザー環境では install 未達で偶発的に無傷だった (到達していれば確実に破壊)。全書換 + num 出典 pin 3 本
- 欠陥 D [中 = 潜伏破壊]: HEAD 注入器 (prepend_invokestatic_to_method) の精密再検証。**相対分岐オフセットは offset 0 挿入で全命令一様シフトのため数学的に不変** (仮説「分岐未再配置」は計算で反証) — 真の欠陥は 3 点: (1) tableswitch/lookupswitch のオペランドがコード先頭基準 4byte アライン依存 → +3 で全テーブル解釈破壊 (再パディングは長さ変化連鎖のため v1 保守的拒否 = 嘘の注入禁止)、 (2) 先頭スタックマップフレームの delta+3 が same_frame/same_locals 1byte 表現 (≤63) 溢れ時に旧コードが**無言スキップ**して破壊 (= VerifyError 相当、昇格未実装のため事前拒否)、 (3) 先頭 append_frame (252-254) が bump 対象外。根治: 命令長ウォーカー (ldc オペランド等の 0xAA 誤検回避のため命令境界走査・解析不能は保守的拒否) で switch 含有拒否 + first_frame_delta_overflow 事前拒否 + append bump 追加。pin 4 本 (fixture 手組)
- 根治 (E): **RetransformClasses 追撃** — ブリッジ準備後に静的 TARGET 11 + dynamic snapshot を loader.loadClass で収集し RetransformClasses (rc/件数ログ) = install 前ロード済み分の後追いパッチ
- 根治 (F = ユーザー提案採用): **F3 マーカー** — 実行時リフレクション picker (1.21.11 実在確認済候補 DebugScreenEntryList → DebugScreenOverlay fallback、0 引数・java.util.List 返却・mods abstract|native 除外・**ジェネリクス要素型が String/Component のみ受理** — List<DebugScreenEntry> 等ロジックリストへの String 混入は消費側キャストで CCE になるため判別不能は不注入の正直スキップ) で F3 行メソッド特定 → f3_marker 新設 (registry + patcher 統合) → ClassRewriter::inject_tail_list_marker (areturn 直前: String 要素は dup/ldc_w marker/invokeinterface List.add/pop の **10 byte**、Component 要素は Component.literal 介し **13 byte**、max_stack += 2 厳密十分量)。v1 保守条件: desc `)Ljava/util/List;` 終端・非 abstract/native・最終命令 areturn・exception_table 空・StackMapTable 全フレーム絶対 offset < 挿入点 (walker 実装、次フレーム = prev+delta+1 正典計算)・code+注入長 ≤65535・LNT 挿入点以降のみ追随 — 一つでも不合なら理由付き拒否。マーカー文 = **`RsGraphics Render (Rsift)`** (ユーザー指定語句を正確に含有)。登録後 RetransformClasses で即時適用
- 根治 (G = 「Vanilla判定」直接応答): **ウィンドウタイトルマーカー** — Minecraft.getInstance→getWindow→setTitle で `Minecraft* 1.21.11 - Rsift (RsGraphics Render)` (バニラ様式 `Minecraft* <ver>` 接尾、Mod UI はマイクラ味方針に整合)。client_tick で成功するまで冪等リトライ
- TDD/pin: parser **3 → 19 (+16)**: walker/head 4 (tableswitch 検出 + ldc 0xAA 誤検回避・拒否・happy bump・delta overflow 拒否) + tail 7 (String/Component happy・冪等・例外表拒否・挿入点フレーム拒否・非 areturn 拒否・非 List 返却拒否) + stackmap 絶対 offset walker 1 + f3_marker 4。jvm **3 → 15 (+12)**: jvmti num 出典 pin 3 + agent_opts mod_dir 5 (arg 最優先/cwd 優先/不在時 dll fallback/死んだ opts fall-through/no-dll 相対) + agent_bridge 純粋 4 (classify String/Component/raw/Integer/f3 スコアリング/称号フラグ不変式/vm addr 整列)。setup 39・rszoom 9・api 82・opt-gfx 1485 全緑維持
- adversarial 6 変異: (1A) IDX_SET_EVENT_CALLBACKS 121→122 (旧欠陥方向) → jvmti pin 1 RED (1B) CALLBACK_SLOT 4→54 (イベント番号混同 = 旧欠陥値) → 1 RED (2A) tail_inject_len String 10→9 → 3 RED (2B) stackmap 絶対計算 prev+delta+1 → prev+delta → walker 1 RED (3A) cwd 候補 is_dir 除去 → 3 RED (3B) modDir arg キー破壊 → 3 RED。全復元 MD5-VERIFIED×6
- fmt 自己起因 0 (編集 8 ファイル全て rustfmt in-place 正規化で恒久 0 化、agent_opts 4/screen_inject 30/class_rewriter 6 の HEAD 原生逸脱も同時解消)・簡体字 0 (seal san 28 files 0 findings)・seal 全 6 ゲート PASS (digest 004c1cf5fb17bfe8 rows=357 不変)
- 誠実境界: (i) sandbox に java/javac/jshell 無し = 実 JVM での install/capture/retransform/注入の実行検証は不可能、ユーザー環境での次回起動が真の検定 (rsift-bootstrap.log 送付を継続依頼) (ii) F3 行メソッドの reflection 特定に失敗した場合はマーカー不注入で理由ログ (タイトルマーカーのみ) = 嘘の注入禁止の設計 (iii) 1.21.11 実機でのタイトル/F3 表示の目視確認はユーザー検証待ち (iv) HEAD 注入の不保守拡張 (switch 再パディング/same_frame_extended 昇格) は v1 保守拒否を維持 = 将来 wave の選択肢として記録 (v) getLoadedClasses 掃引経路は負荷抑制のため 10 秒周期 (捕捉経路が本命でこちらは保険)

## wave 220 HQ (2026-08-03) — cpu_saver §7 違反の根治 = branchless_dda への真配線 (車輪の再発明 dedup) + scan_tiled 不可能証明削除

- 対象: rsift-opt-gfx/src/cpu_saver.rs (モジュール全体) + branchless_dda.rs (inv_dir/branchless_axis/trace_section)
- 選定経緯: §7 (2026-07-26)「未配線・スタブ・消費者ゼロ保持の全面禁止」の系統的再監査。リポジトリ全体の `pub fn` 消費者ゼロ機械スキャン (1221 件中 251 件候補) → モジュール単位の外部参照ファイル数で絞り込み → iris_pipeline/cpu_saver/vertex_compression_r10g10 が候補。**精査**: iris_pipeline・vertex_compression は glob 再エクスポート経由の実消費 (rsift-launcher/half_vertex 等) で偽陰性と排除、cpu_saver のみ**項目別 lexeme 照合でも外部消費者ゼロ**と機械確定
- **HQ-1 [中] §7 違反確定 + 真因**: wave 114 DN-5 が cpu_saver を「将来ホットループ向けプリミティブとして**保持**」とした状態は §7 違反。真因 = cpu_saver モジュール doc 自身が「DDA 本体は branchless_dda 側が担う」と明記するのに、branchless_dda は stepper/選択プリミティブを**局所再実装**していた (HK-1 と同型の車輪の再発明)。branchless_dda::branchless_axis は cpu_saver::advance_axis と**意味論完全同値** (手証明: 同値タイ優先 x>y>z が step_x/step_y/step_z の各 boolean 式で一致・t1<=tmy 下で (t0>t1)||(t0>t2)⟹t0>t1 により step_y=旧 !a0&&a2) をコピー、step 計算も step_direction と観測等価 (d=0/-0.0 の差分は tiny 帯 (DE-1) で step 未使用)
- **HQ-2 [中] 根治 = §7 option(a) 真配線 (branchless_dda 統合・4 点・全挙動保存)**: (1) `branchless_axis` 本体を `advance_axis` 委譲へ (dedup) (2) trace_section の step 配列を `step_direction` へ (branchless_select_i32 伝播消費化) (3) inv_dir の tiny±INF 選択を `branchless_select_f32` へ (branchless_select_u32 伝播消費化・bit 厳密) (4) DDA ループ末尾に次反復読出し先 `CacheLinePrefetcher::prefetch_read` を配線 (clamp で常時 [0,16)^3 in-bounds ポインタ・DN-4 フォールトフリー・効果は非観測だが実呼出として真消費者化)。これで advance_axis/step_direction/branchless_select_{i32,f32,u32}/prefetch_read の 6 アイテムが実消費者獲得
- **HQ-3 [低] scan_tiled = §7 option(b) 不可能証明削除**: `LoopTiledVoxelScanner::scan_tiled` は非退化の真消費者が機械証明上不存在のため削除 (git 履歴残置)。不可能証明 3 点: (1) タイル前提は voxel=1B 依存 (DN-1) だが SectionPalette=[u16;4096] は 2B/voxel で 4³=64voxel=128B=2 キャッシュライン = 64B 一致前提破綻 (2) ホット 16³ 走査 (meshing) は順序依存で digest 拘束 (3) 順序非依存削減 (section_all_air) は既にシーケシャル自動ベクトル化済で tiled 順は局所性劣化 = 性能退化。テスト 2 件も削除
- 検証 (全て実測): lib **1493→1492** (−2 scan_tiled +1 wave-HQ 委譲 pin = 正味 −1) 全緑・**digest 不変 004c1cf5fb17bfe8 rows=357** (release・挙動保存の金標準)・clippy 編集ファイル 0 警告。新 pin `branchless_axis_delegates_to_cpu_saver_advance_axis_equivalence` (500 bit-pattern fuzz + 4 タイケース)
- adversarial 3 系統全 RED + 復元 MD5-VERIFIED×3: (A) advance_axis `<=`→`<` タイ優先反転 → cpu_saver::advance_axis_exactly_one pin RED (DDA fuzz は緑 = タイ優先が「面ちょうど入射では幾何学的にどの軸でも正しい」DDA doc 通り無関係の裏付け・配線安全性の証) (B) step_direction→0 → branchless_dda 7 テスト RED (hits_center/single_block_enclosure_fuzz/slab_entry/negative_tiny/integer_origin/max_steps/negative_direction_slab) (C) branchless_select_f32 真偽入替 → branchless_dda::inv_dir_tiny_sign pin + cpu_saver::select pin 両 RED
- 誠実境界: (i) prefetch の効果はハードウェア状態依存で非観測 (DN-4 契約・adversarial で中性確認済) = 偽配線ではなく実呼出の真消費者化 (ii) cpu_saver は lib.rs `pub use cpu_saver::*` で公開 API 面として他クレートから参照可能だが実呼出ゼロ was the 違反 → branchless_dda 内部配線で解消 (iii) scan_tiled は将来 1B/voxel 等の異レイアウト + 反復 HOT 走査導入時に git 履歴から再導入可能
