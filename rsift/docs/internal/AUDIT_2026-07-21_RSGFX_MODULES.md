...
## wave 217 HN (2026-08-03) — RD24 シナリオの機械スケーリング (実体 40,016 体/編集 7,980 回) + diff-slot OOM 根治 + 「RD24 実体歪み」誠実境界の解消 + 本番配線忘れ (RsSliceFillGuard/NullBlock) の発見
...
- 次 wave 直結の発見: `wide_static_bench` 走査で**本番 wiring 未接続 2 件**を機械発見 — (a) `RsSliceFillGuard::fill_dst(dst, 0xAB)` (wiring.rs) スタブ (b) `CULLIANCE_NULL_BLOCK` (sec_wrap.rs) `#[cfg(any())]` 永久 false。wave 218 で配線 or 削除の本丸対処とする (スタブ/配線なし禁止規律)
