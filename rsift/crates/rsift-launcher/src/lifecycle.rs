...
use rsift_api::{ModRegistry, BumpArena, AdaptivePerfEngine, fabric_api::init_fabric_api};
use rsift_jvm::{JvmConfig, ManagedJvm};
use rsift_render::{Dx12RenderBridge, LwjglProxy, FrameSyncRecorder};
...
impl RsiftOrchestrator {
    pub fn new(mod_dir: PathBuf, jvm_config: JvmConfig, width: u32, height: u32) -> Self {
        ...
        info!(
            ...
            hw.tier.label(), rp.bump_arena_mb, rp.render_distance
        );
        Self {
    ...
    pub fn run(&mut self, game_args: Vec<String>) -> Result<(), String> {
        ...
        info!(
            ...
            hw.tier.label(), hw.cpu_cores, hw.ram_gb, hw.gpu_name
        );
        info!(
        ...
        info!("Phase 1a: Initializing Fabric API ({} modules)...", rsift_api::fabric_api::fabric_api().module_count());
        if let Ok(n) = init_fabric_api() {
            info!("Fabric API: {}/{} modules active", n, rsift_api::fabric_api::fabric_api().module_count());
        }

        // 2. ネイティブDLL Modの検出とロード (ModなしでもOK)
        info!("Phase 1b: Discovering & Loading DLL Mods (optional, ext=.{})...", rsift_api::platform_extension());
        {
        ...
        if std::path::Path::new("minecraft_1.21.11.jar").exists() {
            ...
            info!("Phase 4: Launching Minecraft 1.21.11 — compute redirected to RsCalc native bridge");
    ...
    fn run_hyper_opt_benchmark(&mut self) {
        ...
        info!("Lock-Free RCU zero-copy packet dispatch test: passed={}", passed);
        ...
        let patch_res = rsift_parser::BytecodePatcher::patch_if_needed("net/minecraft/network/Connection", mock_class);
        ...
        info!("[EcoRender] Your tier: {} | Flagship: {}", hw.tier.label(), hw.flagship_boost);
        if let Err(e) = rsift_api::fabric_api::init_fabric_api() {
            error!("Fabric API init failed: {}", e);
        }
        if let Err(e) = rsift_api::neoforge_parity_check::UnifiedDoubleChecker::execute_double_check() {
