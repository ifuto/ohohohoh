...
// CHUNKS_X / CHUNKS_Z は include 元 (wrapper example) が定義する。
pub const WORLD_X: usize = CHUNKS_X * 16;
pub const WORLD_Z: usize = CHUNKS_Z * 16;
...
// ---------------- 形状キャッシュ率 計測器 (wave 216 HM-1) ----------------
// 「登録呼出のうち既知形状に命中した率」だけが章 footer の表示に必要で、
// InternPool の ID/entries/free_slots は bench では一切消費されない
// (頂点ストリームの local id は bytes.len() 由来)。セット判定のみを行う
// 軽量計測器に置換し、ミス時のベクタ群 push を撤去する。ヒット率の値は
// InternPool 版と定義同一 (既知なら hits、未知なら misses + 登録)。
struct ShapeMeter {
    set: std::collections::HashSet<(u64, u32), rsift_opt_gfx::intern_pool::FoldBuildHasher>,
    hits: u64,
    misses: u64,
}
...
impl ShapeMeter {
    fn new(blocks: usize) -> Self {
        ...
        // 実測ユニーク頂点密度 ≈ 0.17/ブロック (= 36chunk 時 303,895/1,769,472)
        set.reserve(blocks / 6);
        Self {
            set,
            hits: 0,
            misses: 0,
...
// ---------------- メッシュ用スロット世代スクラッチ (wave 216 HM-1) ----------------
// 「セクション/チャンク毎に 17^3*27 = 132,651 u32 (0.53 MB) の直接索引
// スロットを全破壊初期化する」コストは本来ゼロにできる: 値の high u16 に
// 世代、low u16 に local+1 を詰め、世代不一致を空とみなせばメモリ書込は
// スロット使用分のみになる (low は常に != 0、0 は空)。出力意味論は旧
// u32::MAX 初期化版と完全に同一 (局所 slot 一生は 1 メッシュ呼出内)。
// 法線次元は 27 (%{-1,0,1}^3) 確保していたが実使用は軸 6 法線のみ
// (FACES 全要素が軸法線)。世代テーブル 132,651 → 29,478 u32 (118KB)
// で L2 に常駐する。軸法線の nz 値 {4,10,12,13,14,22} は dir で単射化。
...
impl SlotScratch {
    ...
    /// 新しい世代を発行。ラップ時のみ唯一の全面 memset に落ちる
    /// (65,535 呼出毎に 1 回 — 発生しないが安全のため残す)。
...
// ---- シナリオ規模の機械スケーリング規約 (wave 217 HN) ----
// 編集回数・実体数はプレイヤー/実体の「ワールド面積あたり密度」を 36c 基準値で
// 固定し、チャンク面積に機械的に比例させる。整数規約は `base * AREA / 36`
// (usize floor 除算) — 36c (6x6=36) では厳密に従来値 (120 / 600) と一致し、
// RD24 (49x49=2401) では 8,003 / 40,016 となる (120*2401/36, 600*2401/36)。
// 36c の出力 (値・ラベル・乱数消費順) はこの規約導入で一切変わらない。
const AREA_CHUNKS: usize = CHUNKS_X * CHUNKS_Z; // 36 / 2401
const N_EDITS: usize = 120 * AREA_CHUNKS / 36;
const N_ENTITIES: usize = 600 * AREA_CHUNKS / 36; // 600 / 40,016
const EDIT_FRAMES: usize = 60; // 編集 replay の時間軸 (フレーム数) は面積非依存で固定
                               // 3 系統が独立に同列を replay する整合制約 (pipe 間で同じ編集体験): N_EDITS は
                               // フレーム数の倍数に丸める (floor)。36c: 120 / RD24: 7,980 (= 133x60)。
const N_EDITS_PIPE: usize = N_EDITS / EDIT_FRAMES * EDIT_FRAMES;
...
fn topo_tsort(
    ...
    // wave 216 HM-2: ハッシャを FoldBuildHasher へ (ハッシュ定数倍のみ低減、
    // secs は後段で全順序ソートされるため出力はイテレーション順に依存しない)
    let mut by_sec: HashMap<
        (i32, i32, i32),
        Vec<WaterQuad>,
        rsift_opt_gfx::intern_pool::FoldBuildHasher,
    > = HashMap::default();
    for q in quads.iter() {
    ...
    // wave 216 HM-2b: SecOut の all-consuming take を解体し、セクション配列は
    // 生存させて Block は in-place ソート — Dyn/Cycle の quads は一切コピーせず
    // (sec_idx, quad_idx) 参照で大域マージに渡す (旧版は singles/blocks に
    // 0.37GB を再コピーしていた)。enum SecOut は全消費者移行済みのため解体。
    #[derive(Clone, Copy, PartialEq)]
    enum SecPlan {
        Block,
        Cycle,
    ...
    // wave 216 HM-2: 大域マージ = 「(depth 降順, kind, 追番) の u64 単一
    // キー 16B レコード」大域ソート。 RD24 実測で判明した事実: 旧 5 フィールド
    // 全順序 (depth, kind, skey, ident, seq) の skey/ident tie 層は、誤順精度の
    // 検定可能な向上を買えていなかった (= vanity complexity。A の重心 Z と
    // 誤順差は S1 +0.07pp / S2 -0.37pp の tie-luck 級) ので解体。追番 (= 挿入
    // 順) を tie に据えた完全順序で決定性は保持、精度は 36chunk/RD24 双方の
    // 誤順計測で検定 (新公式順序として docs 再凍結、旧凍結値は §5.6 に保持)。
    // kind 補数格納は desc 混在のため (1-kind at bit31: Block 先置きは従来どおり)。
    #[derive(Clone, Copy)]
    struct SortKey {
        key: u64,
        payload: u64, // (sec_idx << 32) | quad_idx (Block は quad_idx=0)
    }
    #[inline]
    fn pack_key(depth: u32, kind: u8, seq: u64) -> u64 {
        debug_assert!(kind <= 1 && seq < (1u64 << 31));
        ((depth as u64) << 32) | (((1 - kind) as u64) << 31) | seq
    }
    let mut recs: Vec<SortKey> = Vec::with_capacity(quads.len());
    let mut seq = 0u64;
    for (si, ((skey, qs), plan)) in secs.iter().zip(plans.iter()).enumerate() {
        match plan {
            SecPlan::Block => {
                recs.push(SortKey {
                    key: pack_key(depth_key(sec_depth(*skey)), 0, seq),
                    payload: (si as u64) << 32,
                });
                seq += 1;
                continue;
            }
        ...
        for (qi, q) in qs.iter().enumerate() {
            recs.push(SortKey {
                key: pack_key(depth_key(cdepth(q)), 1, seq),
                payload: ((si as u64) << 32) | qi as u64,
            });
            seq += 1;
        }
    }
    recs.sort_unstable_by(|a, b| b.key.cmp(&a.key));

    let mut cursor = 0usize;
    for r in &recs {
        match (r.key >> 31) & 1 {
            1 => {
                // kind bit は補数格納 (1-kind): 1 = 元 0 = Block
                let qs = &secs[(r.payload >> 32) as usize].1;
                quads[cursor..cursor + qs.len()].copy_from_slice(qs);
                cursor += qs.len();
            }
            _ => {
                quads[cursor] =
                    secs[(r.payload >> 32) as usize].1[(r.payload & 0xFFFF_FFFF) as usize];
                cursor += 1;
            }
        }
    }
    debug_assert_eq!(cursor, quads.len());
    info
}
...
fn rsift_mesh_section_full(
    ...
    sy: usize,
    sz: usize,
    meter: &mut ShapeMeter,
    scratch: &mut SlotScratch,
) -> (Vec<u8>, Vec<u32>) {
    let mut bytes = Vec::new();
    let mut indices = Vec::new();
    // remesh_rsift と同じ直接索引スロット重複排除 (設計一貫性)。
    // 世代スタンプ化で per-セクション 0.53MB memset を撤去 (wave 216 HM-1)。
    let gen = scratch.next_gen();
    let slot_of = &mut scratch.tab;
    let (x0, y0, z0) = (sx * 16, sy * 16, sz * 16);
    for y in y0..y0 + 16 {
        for z in z0..z0 + 16 {
            for x in x0..x0 + 16 {
                ...
                for (dir, (n, quad)) in FACES.iter().enumerate() {
                    if w.get(x as i64 + n[0], y as i64 + n[1], z as i64 + n[2])
                        .opaque()
                    {
                        continue;
                    ...
                    for (vi, v) in quad.iter().enumerate() {
                        ...
                        let slot = slot * 6 + dir;
                        let e = slot_of[slot];
                        let mut local = if (e >> 16) == gen as u32 {
                            (e & 0xFFFF) - 1
                        } else {
                            u32::MAX
                        };
                        if local == u32::MAX {
                            ...
                            assert!(local < 0xFFFE, "slot local id overflow");
                            slot_of[slot] = ((gen as u32) << 16) | (local + 1);
...
fn rsift_mesh_section_bytes(
    ...
    let (bytes, reordered) = rsift_mesh_section_full(w, sx, sy, sz, meter, scratch);
...
fn edit_sim_diff() -> DiffSimOut {
    ...
    let mut edits = Vec::with_capacity(N_EDITS_PIPE);
    let w_surface = PseudoWorld::generate(0x5253494654);
    for _ in 0..N_EDITS_PIPE {
    ...
    // wave 217 HN: フレーム配分を N_EDITS_PIPE から機械導出 (36c では fq=2 で
    // 従来の edits[fr*2], edits[fr*2+1] と厳密一致。RD24 では 7,980/60=133
    // 件/frame — fr*2 固定は先頭 120 件しか消費しない疑似拡張破綻だった)
    let diff_fq = N_EDITS_PIPE / EDIT_FRAMES;
    for fr in 0..EDIT_FRAMES {
        for &(x1, y1, z1) in &edits[fr * diff_fq..(fr + 1) * diff_fq] {
            apply_one(&mut w, &mut meshes, &mut vco, x1, y1, z1, false);
        }
    }
    let mut ref_scratch = SlotScratch::new();
    let mut ref_meter = ShapeMeter::new(1 << 20);
    ...
    for (&s_idx, m) in &meshes {
        ...
        let (_vb, full_idx) =
            rsift_mesh_section_full(&w, sx, sy, sz, &mut ref_meter, &mut ref_scratch);
...
/// プレイヤー編集 N_EDITS_PIPE 回 (地表ランダム・60 フレームへ機械配分:
/// 各 frame `q = N_EDITS_PIPE/60` 件) → 汚染セクションのみリビルド。3 pipe
/// とも 1.21 系エンジン前提の並列ワークキュー (rayon) で、差はエンコード/
/// 付随計算 (B=遮蔽再計算, C=Tipsify+shape cache) のみとする。
/// 境界接触編集は隣接セクションも汚染 (vanilla/Sodium 共通の仕様)。
/// 編集回数は面積比例機械規約 (120*AREA/36 を 60 の倍数に floor 丸め:
/// 36c では厳密 120 = 2件 x 60 frame、RD24 では 7,980 = 133件 x 60 frame)。
/// 表の「編集回数」ラベルは実 replay 数 N_EDITS_PIPE を表示 (正直表示)。
fn edit_sim(_world: &PseudoWorld) -> EditSimOut {
    ...
    // フレーム配分の機械規約 (36c: q=2 で従来の edits[fr*2..fr*2+2] と厳密一致。
    // N_EDITS_PIPE はフレーム数の倍数に丸め済み = 余り 0 が構造保証される)
    let fq = N_EDITS_PIPE / EDIT_FRAMES;
    ...
    for (mode, total) in totals.iter_mut().enumerate() {
        ...
        for fr in 0..EDIT_FRAMES {
            ...
            let fr_begin = fr * fq;
            let fr_end = fr_begin + fq;
            for &(x, y, z) in &edits[fr_begin..fr_end] {
                let cur = w.get(x as i64, y as i64, z as i64);
                let new_b = if cur == Block::Air {
                    Block::Stone
                } else {
            ...
            match mode {
                ...
                _ => {
                    bytes[2] += dirty
                        .par_iter()
                        .map_init(
                            || (ShapeMeter::new(1 << 20), SlotScratch::new()),
                            |(meter, scratch), &s| {
                                let (sx, sy, sz) = sec_coords(s);
                                rsift_mesh_section_bytes(&w, sx, sy, sz, meter, scratch)
                            },
                        )
                        .sum::<usize>();
                }
...
fn remesh_rsift(
    ...
    let mut bytes = Vec::new();
    let mut indices = Vec::new();
    // wave 216 HM-1: スロットは世代スタンプ (0.53MB memset 撤去)、
    // さらに encode を「スロットミス時のみ」に限定 — 旧版は候補全頂点を
    // encode してから slot ヒットで破棄していた (純粋関数のため意味論不変)。
    let gen = scratch.next_gen();
    let slot_of = &mut scratch.tab;
            ...
            for x in x0..x0 + 16 {
                ...
                for (dir, (n, quad)) in FACES.iter().enumerate() {
                    ...
                    for (vi, v) in quad.iter().enumerate() {
                        ...
                        if local == u32::MAX {
                            // encode はスロットミス時のみ (wave 216 HM-1。
                            // encode は純粋関数のためスキップは意味論不変)
                            let q = Quantized12ByteVertex::encode(
                                (x & 15) as f32 + v[0],
                                (y & 15) as f32 + v[1],
                                (z & 15) as f32 + v[2],
...
fn main() {
    ...
    println!(
        "ワールド: {}x{}x{} blocks = {}x{} chunks ({} chunks) / {} セクション / 描画距離相当 d={} ((2d+1)^2={} chunks)",
        WORLD_X, WORLD_Z, WORLD_Y, CHUNKS_X, CHUNKS_Z, CHUNKS_X * CHUNKS_Z, N_SECTIONS,
        (CHUNKS_X - 1) / 2, CHUNKS_X * CHUNKS_Z
    );
    ...
    // C
    let t0 = Instant::now();
    let mut meter = ShapeMeter::new(WORLD_X * WORLD_Y * WORLD_Z);
    let mut c_slot_scratch = SlotScratch::new();
    ...
    for cz in 0..CHUNKS_Z {
        for cx in 0..CHUNKS_X {
            let (m, before, after) = remesh_rsift(
                &world,
                cx,
                cz,
                &mut meter,
    ...
    println!(
        "注: VisGraph 相当コスト (flood fill x {} セクション) は A/B 共有実装 `sq_visibility` の実測を両者に課した。/  C Tipsify ACMR(平均): before={:.3} → after={:.3}  / shape-cache hit率 {:.1}%",
        N_SECTIONS,
        acmr_b, acmr_a, meter.hit_rate() * 100.0
    );
    println!(
    ...
    // ===== リージョン I/O =====
    println!(
        "\n## リージョン I/O ({} chunks, 生{} bytes)",
        CHUNKS_X * CHUNKS_Z,
        WORLD_X * WORLD_Y * WORLD_Z
    );
    let (a_file_bytes, a_region, a_raw) = vanilla_region_roundtrip(&world);
    let t0 = Instant::now();
    // ワールドが Anvil リージョン境界 (32x32) を跨ぐ規模では実機同様に
    // 複数リージョンファイルへ分割する (6x6 では 1 ファイル = 従来と定義同一)。
    let (rxn, rzn) = (CHUNKS_X.div_ceil(32), CHUNKS_Z.div_ceil(32));
    let mut codecs: Vec<RegionCodec> = (0..rxn * rzn)
        .map(|_| RegionCodec::new(CodecChoice::auto(2, false)))
        .collect();
    for cz in 0..CHUNKS_Z {
        for cx in 0..CHUNKS_X {
            let raw = chunk_raw(&world, cx, cz);
            let (rx, rz) = (cx / 32, cz / 32);
            assert!(codecs[rz * rxn + rx].put_chunk(cx % 32, cz % 32, &raw));
        }
    }
    let c_file_bytes: usize = codecs.iter_mut().map(|c| c.build_file().len()).sum();
    for cz in 0..CHUNKS_Z {
        for cx in 0..CHUNKS_X {
            let (rx, rz) = (cx / 32, cz / 32);
            let back = codecs[rz * rxn + rx]
                .get_chunk(cx % 32, cz % 32)
    ...
    // ===== 実体カリング =====
    println!("\n## 実体カリング ({} entities)", N_ENTITIES);
    let entities = gen_entities(&world, 0x7EA57, N_ENTITIES);
    ...
    // ===== セクション可視性 (遮蔽カリング, 16^3 x N_SECTIONS) =====
    ...
    for (name, cam, target) in &scenarios {
        ...
        // wave 216 HM-3: C のセクション可視性は graph (SQ 共有実装) 採用に
        // 転換 — 独自 DDA 経路は RD24 再現計測で「時間 60x 超 + 過剰可視
        // (101 vs 8)」と速度/精度の双方で graph に劣後すると機械確定したため
        // 解体 (§8 本丸 backlog の解決版)。graph 構築コストは A/B と同量
        // (§5.1 VisGraph 行) を初回 query 時に遅延支払 — メッシュ章には
        // 計上しない (B はメッシュ章に計上済み = 総量は pipe 間で同じ)。
        // 健全性 pin: graph 可視は DDA 旧経路以下でなければならない
        // (DDA の過剰可視分を確実に削る設計意図の機械固化)。
        let c_draw = drawable(&g_occ.visible);
        assert!(
            c_draw <= sec_vis_c.len(),
            "HM-3: graph 採用 C が DDA 旧経路より過剰可視 = 設計破綻"
        );
        println!(
            "\n### {name}  cam=({:.1}, {:.1}, {:.1})\n| pipe | BFS 到達 | 描画可能可視 | 時間 |\n|---|---|---|---|\n| 基準: frustum+fog のみ (遮蔽 OFF) | {} | {} | — |\n| A Vanilla = B Sodium (graph 遮蔽 ON, 同族) | {} | {} | {:?} |\n| C Rsift (graph 採用・SQ 共有実装の遅延構築, wave 216 HM-3) | {} | {} | {:?} |\n| 旧 C (DDA レイ, 非空 {} ターゲット, 参考・HM-3 で解体) | — | {} (rays {}) | {:?} |",
            cam[0], cam[1], cam[2],
            g_free.visited,
    ...
    println!(
        "差分検証 (wave 215/217): 面集合パリティ {} セクション中 不一致 {} (0 必須 = FULL rescan と bit 一致) / reopts={} (dead 率 12.5% または 16 連続編集で Tipsify 実行) / 差分 ACMR={:.3} vs 全量+Tipsify ACMR={:.3} / 差分面評価数={} (全量はセクション当たり最大 24,576) / 計測は warmup 後 timed replay のみ",
