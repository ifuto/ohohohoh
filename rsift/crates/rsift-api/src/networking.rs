...
use crate::packet::{DirectBufferSlice, DirectBufferSliceMut};
...
pub trait PlayChannelHandler: Send + Sync {
    ...
    fn receive(&self, channel: &ChannelId, slice: &DirectBufferSlice, sender: &dyn PacketSender) -> bool;
}
...
impl NetworkManager {
    ...
    pub fn register_server_receiver(&mut self, channel: ChannelId, handler: Arc<dyn PlayChannelHandler>) {
        info!("Registering ServerPlayNetworking receiver for channel [{}]", channel.as_str());
    ...
    pub fn register_client_receiver(&mut self, channel: ChannelId, handler: Arc<dyn PlayChannelHandler>) {
        info!("Registering ClientPlayNetworking receiver for channel [{}]", channel.as_str());
        self.client_play_handlers.insert(channel, handler);
        crate::platform::mark_dirty();
    }

    /// ログイン・ハンドシェイクチャネルを登録 (`ServerLoginNetworking` / `ClientLoginNetworking`)
    pub fn register_login_receiver(&mut self, channel: ChannelId, handler: Arc<dyn PlayChannelHandler>) {
        info!("Registering LoginNetworking receiver for channel [{}]", channel.as_str());
        self.login_handlers.insert(channel, handler);
        crate::platform::mark_dirty();
    }

    /// Nettyの直接メモリから渡された生ポインタを解決してハンドラへディスパッチする
    pub fn dispatch_raw(&self, channel_str: &str, ptr: i64, len: i32, is_client: bool, sender: &dyn PacketSender) -> bool {
        ...
        let slice = match unsafe { DirectBufferSlice::from_raw_jni(ptr, len) } {
            Ok(s) => s,
            Err(e) => {
                warn!("DirectBuffer slice error on channel [{}]: {}", channel_str, e);
                return true;
            }
        };
        ...
        if let Some(handler) = handlers.get(&channel) {
            debug!("Dispatching zero-copy packet on channel [{}] (size: {} bytes)", channel_str, len);
...
impl PacketSender for PlatformPacketSender {
    fn send_packet(&self, channel: &ChannelId, raw_ptr: i64, len: i32) -> Result<(), String> {
        ...
        let slice = unsafe { DirectBufferSlice::from_raw_jni(raw_ptr, len) }
